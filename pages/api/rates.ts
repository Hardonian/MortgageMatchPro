...
const RateQuerySchema = z.object({
  ...
  propertyPrice: z
    .string()
    .transform(Number)
    .pipe(z.number().min(10000).max(50000000)),
  downPayment: z
    .string()
    .transform(Number)
    .pipe(z.number().min(0).max(10000000)),
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { country, termYears, rateType, propertyPrice, downPayment, userId } =
    ...
    if (userId) {
      const { error } = await supabaseAdmin.from("rate_checks").insert({
        user_id: userId,
        country,
...
export default withSecurity(withRateLimit("rates")(handler));
