...
import {
  ResourceManager,
  ResourceType,
} from "../../lib/optimization/resource-manager";
  ...
  try {
    ...
    switch (action) {
      ...
      case "allocate":
        // Allocate resources
        const resourceType =
          (req.query.type as ResourceType) || ResourceType.MEMORY;
      ...
      case "release":
        ...
        const released = resourceManager.releaseResources(
          allocationIdToRelease
        );
        if (!released) {
          return res.status(400).json({
