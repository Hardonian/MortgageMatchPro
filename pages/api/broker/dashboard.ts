...
import { withSecurity, withAuth, handleError } from "@/lib/security";
...
async function handler(
  ...
  try {
    ...
    if (userError || !user || user.subscription_tier !== "broker") {
      return res
        .status(403)
        .json({ error: "Access denied. Broker subscription required." });
    }
    ...
    const pendingLeads =
      leads?.filter((lead) => lead.status === "pending").length || 0;
    ...
    const totalCommission =
      convertedLeads * (broker.commission_rate / 100) * 1000; // Assume $1000 average commission per lead
    const monthlyCommission =
      convertedLeads * (broker.commission_rate / 100) * 1000; // Simplified monthly calculation
