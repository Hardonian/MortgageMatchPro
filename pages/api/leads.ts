  ...
  try {
    ...
    if (!consentToShare || !consentToContact) {
      return res.status(400).json({
        ...
        message:
          "You must consent to share your information and be contacted by lenders",
      });
    }
    ...
    const qualificationResult =
      await leadQualificationService.processLeadQualification(leadInput);

    // Save lead to database
    let leadId: string;
    if (userId) {
      leadId = await leadQualificationService.saveLead(
        userId,
        leadInput,
        qualificationResult
      );
    ...
    const notificationPromises = qualificationResult.brokerRecommendations.map(
      (broker) => {
    ...
    res.status(200).json({
      ...
      message:
        "Lead submitted successfully. You will be contacted by qualified mortgage professionals.",
    });
  } catch (error) {
...
export default withSecurity(
  withRateLimit("leads")(withValidation(LeadInputSchema)(handler))
);
