...
class WatcherNotifier {
  ...
  async notifyWatcherResults(
    ...
    try {
      ...
      summary.system_health_score =
        this.calculateSystemHealthScore(watcherResults);
      ...
      console.log(
        `✅ Watcher notification completed. Overall health: ${summary.overall_health}`
   ...
   */
  private analyzeWatcherResults(
    watcherResults: WatcherResult[]
  ): WatcherSummary {
    const totalWatchers = watcherResults.length;
    const successfulWatchers = watcherResults.filter(
      (r) => r.status === "success"
    ).length;
    const failedWatchers = watcherResults.filter(
      (r) => r.status === "error" || r.status === "critical"
    ).length;

    const totalIssues = watcherResults.reduce(
      (sum, r) => sum + r.issues_found,
      0
    );
    const totalWarnings = watcherResults.reduce(
      (sum, r) => sum + r.warnings_found,
      0
    );
    const totalErrors = watcherResults.reduce(
      (sum, r) => sum + r.errors_found,
      0
    );
    const totalCritical = watcherResults.reduce(
      (sum, r) => sum + r.critical_issues_found,
      0
    );
  ...
  private generateSystemRecommendations(summary: WatcherSummary): string[] {
    ...
    if (summary.total_critical > 0) {
      ...
      recommendations.push(
        "Review all critical issues and implement fixes immediately"
      );
    }

    if (summary.total_errors > 0) {
      recommendations.push(
        "⚠️ System errors detected - address within 24 hours"
      );
      recommendations.push(
        "Implement error handling and monitoring improvements"
      );
    }

    if (summary.total_warnings > 0) {
      recommendations.push(
        "📋 System warnings detected - address within 1 week"
      );
      recommendations.push(
        "Review warning patterns and implement preventive measures"
      );
    }

    if (summary.failed_watchers > 0) {
      recommendations.push(
        "🔧 Some watchers failed to execute - investigate and fix"
      );
    ...
    if (summary.system_health_score < 70) {
      recommendations.push(
        "📊 System health score is low - implement comprehensive improvements"
      );
      recommendations.push(
        "Focus on reducing issues and improving reliability"
      );
    }

    if (summary.system_health_score > 90) {
      recommendations.push(
        "✅ System health is excellent - maintain current practices"
      );
      recommendations.push(
        "Consider implementing additional monitoring for proactive maintenance"
      );
    }

    // Specific recommendations based on watcher types
    const dbWatcher = summary.watcher_results.find(
      (r) => r.watcher_name === "db_integrity"
    );
    if (dbWatcher && dbWatcher.critical_issues_found > 0) {
      recommendations.push(
        "🗄️ Database integrity issues - review data validation and constraints"
      );
    }

    const apiWatcher = summary.watcher_results.find(
      (r) => r.watcher_name === "api_contract"
    );
    if (apiWatcher && apiWatcher.critical_issues_found > 0) {
      recommendations.push(
        "🔌 API contract issues - update documentation and fix endpoints"
      );
    }

    const aiWatcher = summary.watcher_results.find(
      (r) => r.watcher_name === "ai_performance"
    );
    if (aiWatcher && aiWatcher.critical_issues_found > 0) {
      recommendations.push(
        "🤖 AI performance issues - optimize models and implement monitoring"
      );
    }
  ...
  private async storeWatcherSummary(summary: WatcherSummary): Promise<void> {
    try {
      const { error } = await this.supabase.from("watcher_summaries").insert({
   ...
   */
  private async createSystemHealthIssue(
  ...
  private formatSystemHealthIssueBody(summary: WatcherSummary): string {
    const criticalWatchers = summary.watcher_results.filter(
      (r) => r.critical_issues_found > 0
    );
    const failedWatchers = summary.watcher_results.filter(
      (r) => r.status === "error" || r.status === "critical"
    );
...
- **Status:** ${
      summary.system_health_score >= 90
        ? "Excellent"
        : summary.system_health_score >= 70
        ? "Good"
        : summary.system_health_score >= 50
        ? "Fair"
        : "Poor"
    }
- **Trend:** ${
      summary.system_health_score >= 80 ? "Improving" : "Needs Attention"
    }
...
if (require.main === module) {
  ...
  const mockResults: WatcherResult[] = [
    {
      ...
      details:
        "All foreign key constraints are valid, no orphaned records found",
    ...
    },
    {
      ...
      details:
        "Found 3 undocumented endpoints, 1 deprecated endpoint still in use",
      recommendations: [
  ...
  notifier
    .notifyWatcherResults(mockResults)
    .then((summary) => {
