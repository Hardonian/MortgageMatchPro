...
class AIPerformanceWatcher {
  ...
  async runPerformanceCheck(): Promise<PerformanceReport> {
    ...
    try {
      ...
      const report: PerformanceReport = {
        ...
        total_requests: modelPerformance.reduce(
          (sum, model) => sum + model.total_requests,
          0
        ),
        total_tokens: modelPerformance.reduce(
          (sum, model) => sum + model.total_tokens,
          0
        ),
        total_cost: modelPerformance.reduce(
      ...
      console.log(
        `✅ Performance monitoring completed. Health: ${overallHealth}`
  ...
  private async collectModelPerformance(): Promise<ModelPerformance[]> {
    ...
    try {
      ...
      for (const [key, group] of Object.entries(modelGroups)) {
        ...
        const totalTokens = g.tokens.reduce(
          (sum: number, t: number) => sum + t,
          0
        );
        const totalCost = g.costs.reduce(
          (sum: number, c: number) => sum + c,
          0
        );
        const avgLatency =
          g.latencies.reduce((sum: number, l: number) => sum + l, 0) /
          g.latencies.length;
        ...
        performance.push({
          ...
          last_used:
            g.requests[g.requests.length - 1]?.created_at ||
  ...
  private calculateAverageLatency(performance: ModelPerformance[]): number {
    ...
    const weightedLatency = performance.reduce(
      (sum, model) => sum + model.avg_latency_ms * model.total_requests,
      0
   ...
   */
  private async analyzeCostTrend(): Promise<
  ...
  > {
    try {
      ...
      const avgLatencies = Object.entries(dailyLatencies).map(
        ([_, latencies]) => {
      ...
      const firstHalf = avgLatencies.slice(
        0,
        Math.floor(avgLatencies.length / 2)
      );
      const secondHalf = avgLatencies.slice(
        Math.floor(avgLatencies.length / 2)
      );

      const firstHalfAvg =
        firstHalf.reduce((sum, lat) => sum + lat, 0) / firstHalf.length;
      const secondHalfAvg =
        secondHalf.reduce((sum, lat) => sum + lat, 0) / secondHalf.length;
    ...
    for (const model of performance) {
      ...
      if (model.total_cost > 50) {
        alerts.push({
          ...
          model: model.model,
          message: `High cost detected: $${model.total_cost.toFixed(
            2
          )} in 7 days`,
          ...
          recommendation:
            "Consider cost optimization strategies or model alternatives",
        });
      }
      ...
      if (model.total_requests < 10) {
        alerts.push({
          ...
          recommendation:
            "Consider removing unused models or increasing adoption",
        });
      }
  ...
  private generateRecommendations(
    ...
    if (criticalAlerts.length > 0) {
      recommendations.push(
        "🚨 Critical AI performance issues detected - immediate attention required"
      );
    ...
    if (highAlerts.length > 0) {
      recommendations.push(
        "⚠️ High priority performance issues found - address within 24 hours"
      );
    ...
    if (highCostModels.length > 0) {
      recommendations.push(
        "💰 High-cost models detected - consider optimization strategies"
      );
      recommendations.push(
    ...
    if (slowModels.length > 0) {
      recommendations.push(
        "⏱️ Slow models detected - consider performance optimizations"
      );
    ...
    if (errorProneModels.length > 0) {
      recommendations.push(
        "🔧 High error rate models detected - implement error handling"
      );
    ...
    if (underusedModels.length > 0) {
      recommendations.push(
        "📊 Underutilized models detected - consider consolidation"
      );
...
${report.model_performance
  .map(
    (model) =>
      ...
      } requests, $${model.total_cost.toFixed(2)} cost, ${
        model.avg_latency_ms
      }ms avg latency, ${model.error_rate}% error rate`
  )
  .join("\n")}
