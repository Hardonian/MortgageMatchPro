...
export class PerformanceMonitor {
  ...
  recordMetric(
    metric: Omit<PerformanceMetrics, "timestamp" | "memoryUsage" | "cpuUsage">
  ): void {
  ...
  private checkThresholds(metric: PerformanceMetrics): void {
    ...
    if (metric.duration > this.thresholds.maxDuration) {
      violations.push(
        `Duration exceeded: ${metric.duration}ms > ${this.thresholds.maxDuration}ms`
      );
    }

    if (metric.memoryUsage.heapUsed > this.thresholds.maxMemoryUsage) {
      violations.push(
        `Memory usage exceeded: ${metric.memoryUsage.heapUsed} bytes > ${this.thresholds.maxMemoryUsage} bytes`
      );
    }

    if (metric.requestSize > this.thresholds.maxRequestSize) {
      violations.push(
        `Request size exceeded: ${metric.requestSize} bytes > ${this.thresholds.maxRequestSize} bytes`
      );
    }

    if (metric.responseSize > this.thresholds.maxResponseSize) {
      violations.push(
        `Response size exceeded: ${metric.responseSize} bytes > ${this.thresholds.maxResponseSize} bytes`
      );
    }

    if (violations.length > 0) {
      console.warn(
        `Performance threshold violations for ${metric.endpoint}:`,
        violations
      );
    }
  }
  ...
  getStatistics(): {
    ...
    return {
      ...
      thresholdViolations: this.metrics.filter((m) => this.checkThresholds(m))
        .length,
    };
  }
  ...
  getSlowestEndpoints(
    limit = 10
  ): Array<{ endpoint: string; averageDuration: number; maxDuration: number }> {
