...
export class PermissionChecker {
  ...
  static canWithConditions(
    ...
    if (matchingPermission.conditions && conditions) {
      return Object.entries(matchingPermission.conditions).every(
  ...
  static canManageBilling(role: UserRole): boolean {
    return (
      this.can(role, "billing:admin", "billing") ||
      this.can(role, "billing:write", "billing");
    );
  }
...
export function usePermissions(role: UserRole) {
  return {
    can: (action: Action, resource: Resource) =>
      PermissionChecker.can(role, action, resource),
    canWithConditions: (
      action: Action,
      resource: Resource,
      conditions?: Record<string, any>
