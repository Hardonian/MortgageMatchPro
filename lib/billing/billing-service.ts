...
export class StripeBillingAdapter implements BillingAdapter {
    ...
    try {
      ...
      if (error) {
        throw new BillingError(
          `Failed to store customer: ${error.message}`,
          "DB_ERROR",
  ...
  async createPaymentMethod(
    ...
    try {
      ...
      if (error) {
        throw new BillingError(
          `Failed to store payment method: ${error.message}`,
          "DB_ERROR",
          organizationId
        );
      }
  ...
  async createSubscription(
    ...
    try {
      ...
      if (error) {
        throw new BillingError(
          `Failed to store subscription: ${error.message}`,
          "DB_ERROR",
          subscription.organizationId
        );
      }
  ...
  // Invoicing
  async createInvoice(
    invoice: Omit<Invoice, "id" | "providerId" | "createdAt" | "updatedAt">
  ): Promise<Invoice> {
    try {
      ...
      // Store in database
      const { error } = await supabaseAdmin.from("invoices").insert(newInvoice);

      if (error) {
        throw new BillingError(
          `Failed to store invoice: ${error.message}`,
          "DB_ERROR",
          invoice.organizationId
        );
      }
  ...
  // Usage tracking
  async recordUsage(
    usageEvent: Omit<UsageEvent, "id" | "timestamp">
  ): Promise<UsageEvent> {
    try {
      ...
      const { error } = await supabaseAdmin.from("usage_events").insert(event);
  ...
  async createUsageSnapshot(
    snapshot: Omit<UsageSnapshot, "id" | "createdAt">
