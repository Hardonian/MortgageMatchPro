...
export type ExplainabilityResponse = z.infer<
  typeof ExplainabilityResponseSchema
>;
...
export class ExplainabilityService {
  ...
  async explainMatch(
    ...
    try {
      // Analyze key factors
      const keyFactors = await this.analyzeKeyFactors(
        recommendation,
        userContext
      ...
      const explanation = {
        ...
        reasoning,
        keyFactors,
        methodology: options.includeMethodology
          ? this.getMethodology()
          : "Standard mortgage analysis methodology",
        confidence,
  ...
  > {
    ...
    if (userContext.income) {
      const incomeRatio =
        userContext.income / (recommendation.propertyPrice || 1);
      factors.push({
        factor: "Income Stability",
        impact:
          incomeRatio > 0.3 ? "high" : incomeRatio > 0.2 ? "medium" : "low",
        description: `Annual income of $${userContext.income.toLocaleString()} provides ${
          incomeRatio > 0.3 ? "strong" : "adequate"
        } financial foundation`,
        evidence: `Income-to-property ratio: ${(incomeRatio * 100).toFixed(
          1
        )}%`,
      });
    }
    ...
    if (userContext.downPayment && recommendation.propertyPrice) {
      const downPaymentRatio =
        userContext.downPayment / recommendation.propertyPrice;
      factors.push({
        factor: "Down Payment",
        impact:
          downPaymentRatio >= 0.2
            ...
            ? "medium"
            : "low",
        description: `${(downPaymentRatio * 100).toFixed(1)}% down payment ${
          downPaymentRatio >= 0.2
            ? "eliminates PMI and improves terms"
            : "meets minimum requirements"
        }`,
        evidence: `Down payment: $${userContext.downPayment.toLocaleString()} (${(
    ...
    if (userContext.creditScore) {
      factors.push({
        factor: "Credit Score",
        impact:
          userContext.creditScore >= 740
            ? "high"
            : userContext.creditScore >= 680
            ? "medium"
            : "low",
        description: `Credit score of ${userContext.creditScore} ${
          userContext.creditScore >= 740
    ...
    if (userContext.income && userContext.debts) {
      ...
      factors.push({
        ...
        description: `DTI of ${dti.toFixed(1)}% ${
          dti <= 36
            ? "is excellent"
            : dti <= 43
            ? "is acceptable"
            : "may limit qualification"
        }`,
        evidence: `Monthly debts: $${userContext.debts}, Monthly income: $${(
          userContext.income / 12
        ).toFixed(0)}`,
      });
    }
  ...
  private async generateReasoning(
    ...
    let reasoning =
      "This recommendation is based on a comprehensive analysis of your financial profile and current market conditions. ";

    if (highImpactFactors.length > 0) {
      reasoning += `The primary factors supporting this recommendation are: ${highImpactFactors
        .map((f) => f.factor)
        .join(", ")}. `;
    }

    if (mediumImpactFactors.length > 0) {
      reasoning += `Additional considerations include: ${mediumImpactFactors
        .map((f) => f.factor)
        .join(", ")}. `;
    }
    ...
    if (recommendation.type === "fixed") {
      reasoning +=
        "A fixed-rate mortgage provides payment stability and protection against rate increases, which is particularly valuable in the current market environment.";
    } else if (recommendation.type === "variable") {
      reasoning +=
        "A variable-rate mortgage offers lower initial payments and potential savings if rates remain stable or decrease, though it carries rate risk.";
    }
  ...
  private async generateRecommendations(
    ...
    const alternatives = includeAlternatives
      ? this.generateAlternatives(recommendation, userContext)
      : [];
  ...
  private generateSummary(recommendation: any, keyFactors: any[]): string {
    ...
    return `Based on your ${factorText}, this ${
      recommendation.type || "mortgage"
    } option offers the best balance of affordability and terms for your situation.`;
  }
  ...
  private identifyLimitations(userContext: any, marketContext: any): string[] {
    ...
    if (!userContext.creditScore) {
      limitations.push(
        "Credit score not provided - analysis based on estimated credit profile"
      );
    }

    if (!userContext.employmentHistory) {
      limitations.push(
        "Employment history not verified - income stability assumptions used"
      );
    }

    if (!marketContext.rateData) {
      limitations.push(
        "Real-time rate data not available - using estimated rates"
      );
    }
  ...
  private assessExperienceLevel(userContext: any): string {
    if (userContext.previousMortgages && userContext.previousMortgages > 0)
      return "experienced";
    return "first-time";
  }
  ...
  private generateAlternatives(recommendation: any, userContext: any): any[] {
    ...
    alternatives.push({
      ...
      suitability:
        userContext.downPayment &&
        userContext.downPayment < userContext.propertyPrice * 0.2
          ? "high"
          : "low",
    });
