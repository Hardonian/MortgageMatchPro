...
export type EnhancedPromptTemplate = z.infer<
  typeof EnhancedPromptTemplateSchema
>;
...
export const ENHANCED_PROMPT_TEMPLATES: Omit<
...
>[] = [
  {
    ...
    description:
      "Calculate mortgage affordability with clear, structured output",
...
Output JSON:
{
  ...
  "disclaimers": [string]
    ...
    fallbackPrompt:
      "Provide basic affordability guidance with disclaimers about incomplete data.",
    maxTokens: 800,
    temperature: 0.1,
  },
  {
  ...
  missing_data: [
    {
      ...
      handling:
        "Request income information with clear explanation of why it's needed",
      fallbackResponse:
        "I need your income information to calculate affordability accurately. Please provide your annual income.",
    },
    {
      condition: "property_price is 0 or negative",
      handling: "Request valid property price or use market average for area",
      fallbackResponse:
        "Please provide a valid property price, or I can use the average price for your area.",
    },
    {
      condition: "down_payment is missing",
      handling: "Calculate minimum down payment required",
      fallbackResponse:
        "I'll calculate the minimum down payment required for your situation.",
    },
  ],
  ambiguous_input: [
    {
      ...
      fallbackResponse:
        "I see you mentioned multiple loan types. Which is your primary preference: fixed or variable rate?",
    },
    {
      condition: "unclear timeline",
      handling: "Request specific timeline or use default",
      fallbackResponse:
        "When are you looking to secure a mortgage? This helps me provide more relevant recommendations.",
    },
  ],
  error_conditions: [
    {
      ...
      fallbackResponse:
        "I encountered an error with the calculation. Here's general guidance: [basic rules]. Consider using a mortgage calculator for precise numbers.",
    },
    {
      condition: "api_timeout",
      handling: "Use cached data with freshness warning",
      fallbackResponse:
        "I'm using slightly older data due to a connection issue. Here's what I found: [cached data]. Please verify current rates.",
    },
  ],
};
...
export class PromptOptimizer {
  static calculateClarityScore(prompt: string): number {
    ...
    if (prompt.includes("Output JSON:") || prompt.includes("Format:")) {
      score += 0.1;
    }
  ...
  static calculatePredictabilityScore(prompt: string): number {
    ...
    if (prompt.includes("Keep explanation:") || prompt.includes("Provide:")) {
      score += 0.1;
    }
    ...
    if (
      ...
      prompt.includes("words") ||
      prompt.includes("sentences")
    ) {
      score += 0.1;
    }
...
export class TemplateValidator {
  static validateTemplate(template: EnhancedPromptTemplate): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    const clarityScore = PromptOptimizer.calculateClarityScore(
      template.basePrompt
    );
    if (clarityScore < 0.3) {
      errors.push("Prompt clarity score too low");
    }

    const brevityScore = PromptOptimizer.calculateBrevityScore(
      template.basePrompt
    );
    if (brevityScore < 0.3) {
      errors.push("Prompt is too verbose");
    }
