...
export class EnhancedMetricsService {
   ...
   */
  async recordRegionalMetrics(
    metrics: Omit<RegionalMetrics, "timestamp">
  ): Promise<void> {
    try {
      ...
      const regionEntries = this.regionalMetrics.filter(
        (m) => m.region === metrics.region
      );
      if (regionEntries.length > 500) {
        this.regionalMetrics = this.regionalMetrics.filter(
          (m) => m.region !== metrics.region
        );
  ...
  async recordTrainingMetrics(
    metrics: Omit<TrainingMetrics, "timestamp">
  ): Promise<void> {
    try {
  ...
  getAIMetricsSummary(period: "hour" | "day" | "week" = "day"): {
    ...
    const recentMetrics = this.aiMetrics.filter(
    ...
    const averageCost =
      recentMetrics.reduce((sum, m) => sum + m.cost, 0) / totalRequests;
    const averageAccuracy =
      recentMetrics.reduce((sum, m) => sum + m.accuracy, 0) / totalRequests;
    const averageConfidence =
      recentMetrics.reduce((sum, m) => sum + m.confidence, 0) / totalRequests;
  ...
  getRegionalMetricsSummary(): {
    ...
    const globalAverageLatency =
      regions.reduce((sum, r) => sum + r.averageLatency, 0) / regions.length;
  ...
  getTrainingMetrics(modelId: string): {
    ...
    const modelMetrics = this.trainingMetrics.filter(
      (m) => m.modelId === modelId
   ...
   */
  getPlaygroundHistory(limit = 50): DeveloperPlayground[] {
  ...
  private async generateMockResponse(
    ...
    return `Mock response for prompt: "${prompt.substring(
      0,
      50
...
${regionalSummary.regions
  .map(
    (r) =>
      `mortgagematch_regional_latency_seconds{region="${r.region}",country="${
        r.country
      }"} ${r.averageLatency / 1000}`
  )
  .join("\n")}
...
mortgagematch_system_health{status="${health.status}"} ${
...
export const recordRegionalMetrics = (
  metrics: Omit<RegionalMetrics, "timestamp">
) => enhancedMetricsService.recordRegionalMetrics(metrics);

export const recordTrainingMetrics = (
  metrics: Omit<TrainingMetrics, "timestamp">
...
export const createPlaygroundSession = (
  ...
  enhancedMetricsService.createPlaygroundSession(
    prompt,
    model,
    temperature,
    maxTokens
...
export const getSystemHealth = () => enhancedMetricsService.getSystemHealth();
