...
import {
  AI_EVENT_TYPES,
  AIScoringRequestedEvent,
  AIScoringCompletedEvent,
} from "../schemas/ai-events";
import { BILLING_EVENT_TYPES } from "../schemas/billing-events";

// AI Scoring Request Handler
export const handleAIScoringRequested = async (event: {
  data: AIScoringRequestedEvent;
}) => {
  console.log(
    `[AI Handler] Processing scoring request: ${event.data.requestId}`
  );
  ...
  try {
    ...
    console.log(
      `[AI Handler] Completed scoring request: ${event.data.requestId}`
    );
  } catch (error) {
    console.error(
      `[AI Handler] Error processing scoring request: ${event.data.requestId}`,
      error
...
export const handleAIScoringCompleted = async (event: {
  data: AIScoringCompletedEvent;
}) => {
...
export const handleAICostThresholdExceeded = async (event: { data: any }) => {
  console.log(
    `[AI Handler] Cost threshold exceeded for tenant: ${event.data.tenantId}`
  );
...
export const initializeAIEventHandlers = () => {
  ...
  subscribe(
    AI_EVENT_TYPES.COST_THRESHOLD_EXCEEDED,
    handleAICostThresholdExceeded,
    {
      priority: 15,
      maxRetries: 1,
