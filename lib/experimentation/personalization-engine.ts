...
export type ContentPersonalization = z.infer<
  typeof ContentPersonalizationSchema
>;
...
export class PersonalizationEngine {
  ...
  async getPersonalizedContent(
    ...
    try {
      ...
      const contentPersonalization: ContentPersonalization = {
        ...
        personalizedContent,
        personalizationFactors: this.extractPersonalizationFactors(
          profile,
          context
        ),
  ...
  async personalizeMortgageAdvice(
    ...
    try {
      ...
      trackEvent("mortgage_advice_personalized", {
        user_id: userId,
        scenario,
        personalization_factors: this.extractPersonalizationFactors(
          profile,
          context
        ),
      });
  ...
  async predictNextAction(
    ...
    try {
      ...
      const prediction = await this.generateActionPrediction(
        profile,
        currentContext
  ...
  private async calculateUserSegments(profile: UserProfile): Promise<string[]> {
    ...
    if (profile.behaviorPatterns.featureUsage) {
      const featureCount = Object.keys(
        profile.behaviorPatterns.featureUsage
      ).length;
      if (featureCount > 5) {
  ...
  private async applyPersonalizationRules(
    ...
    const applicableRules = Array.from(this.personalizationRules.values())
      .filter(
        (rule) =>
          rule.enabled &&
          this.matchesConditions(rule.conditions, profile, context)
      )
    ...
    for (const rule of applicableRules) {
      for (const action of rule.actions) {
        personalizedContent = await this.applyAction(
          action,
          personalizedContent,
          profile,
          context
  ...
  private async generatePersonalizedAdvice(
    ...
    const prompt = `
      ...
      - Communication Style: ${
        profile.preferences.communicationStyle || "simple"
      }
  ...
  private filterRatesByPreferences(rates: any[], profile: UserProfile): any[] {
    ...
    if (profile.preferences.rateType) {
      filteredRates = filteredRates.filter(
        (rate) => rate.type === profile.preferences.rateType
      );
    }
  ...
  private async generateActionPrediction(
    ...
    const prompt = `
      ...
      User Profile:
      - Segments: ${profile.segments.join(", ")}
      - Recent Activity: ${JSON.stringify(
        profile.behaviorPatterns.recentActivity || {}
      )}
      - Feature Usage: ${JSON.stringify(
        profile.behaviorPatterns.featureUsage || {}
      )}
  ...
  private async generateContextualUI(
    prediction: any,
    profile: UserProfile
  ): Promise<any> {
