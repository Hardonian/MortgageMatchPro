...
export class DistributedRateLimiter {
  ...
  constructor(
    ...
    this.redis = new Redis(
      redisUrl || keyManager.getKey("redis") || "redis://localhost:6379"
    );
  ...
  async checkLimit(
    ...
    switch (config.algorithm) {
      ...
      default:
        throw new Error(
          `Unsupported rate limit algorithm: ${config.algorithm}`
        );
    }
  }
  ...
  private async fixedWindowCheck(
    ...
    const windowStart =
      Math.floor(now / config.windowSizeMs) * config.windowSizeMs;
  ...
  private async tokenBucketCheck(
    ...
    const results = await pipeline.exec();
    const [tokens, lastRefill] = (results?.[0]?.[1] as [string, string]) || [
      "0",
      "0",
    ];
  ...
  private async leakyBucketCheck(
    ...
    const results = await pipeline.exec();
    const [level, lastLeak] = (results?.[0]?.[1] as [string, string]) || [
      "0",
      "0",
    ];
  ...
  async batchCheck(
    ...
    const promises = requests.map(async (request) => {
      try {
        ...
        console.error(
          `Error checking rate limit for ${request.identifier}:${request.endpoint}:`,
          error
        );
      }
    });
  ...
  async getStats(): Promise<{
    ...
    return {
      ...
      memoryUsage: `${
        Math.round((parseInt(memoryInfo) / 1024 / 1024) * 100) / 100
      } MB`,
    };
  }
