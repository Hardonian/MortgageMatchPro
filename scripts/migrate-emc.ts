...
class EMCMigrationManager {
  ...
  async detectPendingSteps(): Promise<EMCStep[]> {
    ...
    try {
      ...
      console.log(
        "No migration_steps table found, checking Prisma migrations..."
      );
    }
    ...
    try {
      ...
      if (Array.isArray(migrationStatus) && migrationStatus.length > 0) {
        console.log(
          `Found ${migrationStatus.length} pending Prisma migrations`
        );
  ...
  private async executeMigrate(step: EMCStep): Promise<void> {
    ...
    const backfillFunction = new Function(
      "supabase",
      "prisma",
      "config",
      step.backfillScript
  ...
  private async executeContract(step: EMCStep): Promise<void> {
    ...
    const daysSinceCreated =
      (Date.now() - step.createdAt.getTime()) / (1000 * 60 * 60 * 24);
    if (daysSinceCreated < this.config.verificationWindowDays) {
      throw new Error(
        `Contract step too recent (${daysSinceCreated.toFixed(
          1
        )} days). Minimum: ${this.config.verificationWindowDays} days`
      );
    }
