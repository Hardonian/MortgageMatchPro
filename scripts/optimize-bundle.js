...
const analyzeBundleSize = (platform) => {
  try {
    ...
    const bundleSize = stats.size;
      ...
      bundleSize > CONFIG.targetBundleSize ? "red" : "green"

    // Check against thresholds
    if (bundleSize > CONFIG.maxBundleSize) {
      log(
        `❌ Bundle size exceeds maximum allowed size (${formatBytes(
      ...
      log(
        `⚠️  Bundle size exceeds target size (${formatBytes(
          CONFIG.targetBundleSize
        )})`,
        "yellow"
      );
...
const analyzeDependencies = () => {
  ...
  try {
    ...
    Object.keys(dependencies).forEach((dep) => {
      if (
        suspiciousDependencies.some((suspicious) => dep.includes(suspicious))
      ) {
...
const main = async () => {
  log(
    `${colors.bold}${colors.magenta}🚀 MortgageMatch Pro Bundle Optimization${colors.reset}\n`
  );
  ...
  log(
    `Average bundle size: ${formatBytes(avgSize)}`,
    avgSize > CONFIG.targetBundleSize ? "red" : "green"
  );
  log(`Target bundle size: ${formatBytes(CONFIG.targetBundleSize)}`, "blue");
  log(
    `Large dependencies found: ${largeDeps.length}`,
    largeDeps.length > 0 ? "yellow" : "green"
  );
