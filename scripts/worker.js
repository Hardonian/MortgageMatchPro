...
class WorkerService {
  ...
  async processNextJob() {
    ...
    try {
      ...
      console.log(
        `[Worker] Completed ${jobType} job: ${jobId} (${duration.toFixed(2)}ms)`
  ...
  startHealthMonitoring() {
    setInterval(() => {
      ...
      const successRate =
        this.stats.processedJobs > 0
          ? (this.stats.processedJobs - this.stats.failedJobs) /
            this.stats.processedJobs
          : 0;

      console.log(
        `[Worker] Health Check - Uptime: ${Math.round(uptime / 1000)}s, Jobs: ${
          this.stats.processedJobs
      ...
      for (const [jobType, stats] of this.jobs) {
        console.log(
          `[Worker] ${jobType}: ${stats.total} total, ${
            stats.successful
          } successful, ${
            stats.failed
          } failed, avg: ${stats.averageDuration.toFixed(2)}ms`
        );
      }
    }, 60000); // Every minute
  }
