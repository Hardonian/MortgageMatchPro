  ...
  try {
    if (CONFIG.target === "local") {
      ...
      if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        throw new Error(
          "SUPABASE_URL and SUPABASE_SERVICE_KEY are required for remote execution"
        );
      }

      // Extract database URL from Supabase URL
      const dbUrl = SUPABASE_URL.replace("https://", "").replace(
        ".supabase.co",
        ""
      );
...
async function main() {
  try {
    ...
    if (!fs.existsSync(CONFIG.completeSchemaFile)) {
      throw new Error(
        `Complete schema file not found: ${CONFIG.completeSchemaFile}`
      );
    }
