...
async function showRegionalMetrics() {
  ...
  console.log(
    `Global Error Rate: ${(summary.globalErrorRate * 100).toFixed(2)}%`
  );
  console.log(
    `Total Active Users: ${summary.totalActiveUsers.toLocaleString()}`
  );
  console.log("");
...
async function showTrainingMetrics() {
  ...
  modelIds.forEach((modelId) => {
    ...
    console.log(
      `  Final Accuracy: ${(metrics.finalAccuracy * 100).toFixed(1)}%`
    );
