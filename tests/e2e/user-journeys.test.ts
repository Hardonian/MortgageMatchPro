  ...
  test.describe("Sign-up to AI Match Journey", () => {
    test("should complete full sign-up to AI match flow", async ({ page }) => {
      ...
      await test.step("User signs up", async () => {
        ...
        await expect(
          page.locator('[data-testid="sign-up-success"]')
        ).toBeVisible();
      });
      ...
      await test.step("User verifies email", async () => {
        ...
        await expect(
          page.locator('[data-testid="email-verified"]')
        ).toBeVisible();
      });
      ...
      await test.step("User completes profile setup", async () => {
        ...
        await expect(
          page.locator('[data-testid="profile-complete"]')
        ).toBeVisible();
      });
      ...
      await test.step("User creates mortgage scenario", async () => {
        ...
        await page.fill(
          ...
          testScenario.propertyValue.toString()
        );
        await page.fill(
          '[data-testid="down-payment-input"]',
          testScenario.downPayment.toString()
        );
        await page.fill(
          '[data-testid="credit-score-input"]',
          testScenario.creditScore.toString()
        );
        await page.fill(
          '[data-testid="income-input"]',
          testScenario.income.toString()
        );
        await page.selectOption(
          '[data-testid="employment-status"]',
          testScenario.employmentStatus

        await page.click('[data-testid="save-scenario-button"]');

        await expect(
          page.locator('[data-testid="scenario-saved"]')
        ).toBeVisible();
      });
      ...
      await test.step("User gets AI mortgage match", async () => {
        ...
        await expect(
          page.locator('[data-testid="recommended-rate"]')
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="monthly-payment"]')
        ).toBeVisible();
        await expect(
      ...
      await test.step("User views detailed AI explanation", async () => {
        ...
        await expect(
          page.locator('[data-testid="explanation-modal"]')
        ).toBeVisible();
  ...
  test.describe("Export to CRM Journey", () => {
    test("should complete scenario export to CRM flow", async ({ page }) => {
      ...
      await test.step("User navigates to scenarios", async () => {
        ...
        await expect(
          page.locator('[data-testid="scenarios-list"]')
        ).toBeVisible();
      });
      ...
      await test.step("User selects scenario for export", async () => {
        ...
        await expect(
          page.locator('[data-testid="scenario-details"]')
        ).toBeVisible();
      });
      ...
      await test.step("User exports scenario to CRM", async () => {
        ...
        await expect(
          page.locator('[data-testid="export-filename"]')
        ).toContainText(".csv");
        await expect(
          page.locator('[data-testid="export-record-count"]')
        ).toBeVisible();
      });
  ...
  test.describe("Billing Event Journey", () => {
    test("should complete billing event flow", async ({ page }) => {
      ...
      await test.step("Admin views tenant billing", async () => {
        ...
        await expect(
          page.locator('[data-testid="tenant-billing-modal"]')
        ).toBeVisible();
      });
      ...
      await test.step("Admin generates invoice", async () => {
        ...
        await expect(
          page.locator('[data-testid="invoice-generated"]')
        ).toBeVisible();

        // Verify invoice details
        await expect(
          page.locator('[data-testid="invoice-amount"]')
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="invoice-period"]')
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="invoice-items"]')
        ).toBeVisible();
      });
      ...
      await test.step("Admin verifies billing event", async () => {
        ...
        await expect(
          page.locator('[data-testid="billing-events-list"]')
        ).toBeVisible();

        // Verify the new billing event
        await expect(
          page.locator('[data-testid="billing-event-0"]')
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="billing-event-0"]')
        ).toContainText("Payment Processed");
      });
    });
  });

  test.describe("Error Handling Journey", () => {
    test("should handle errors gracefully throughout the flow", async ({
      page,
    }) => {
      ...
      await test.step("User experiences network error", async () => {
        ...
        await expect(
          page.locator('[data-testid="offline-error"]')
        ).toContainText("Network connection required");
  ...
  test.describe("Accessibility Journey", () => {
    test("should be accessible via keyboard navigation", async ({ page }) => {
      ...
      await test.step("User navigates using keyboard only", async () => {
        ...
        await expect(
          page.locator('[data-testid="sign-up-form"]')
        ).toBeVisible();
      });
