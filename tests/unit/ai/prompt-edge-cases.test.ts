...
describe("AI Prompt Edge Cases", () => {
  ...
  describe("Missing Data Handling", () => {
    it("should handle missing income data", async () => {
      ...
      const result = await promptTemplates.generatePrompt(
        "affordability",
        incompleteContext
  ...
  describe("API Error Handling", () => {
    it("should handle OpenAI API errors", async () => {
      ...
      await expect(
        modelRouting.routeRequest("test prompt", context, {})
    ...
    it("should handle timeout errors", async () => {
      ...
      await expect(
        modelRouting.routeRequest("test prompt", context, { timeout: 1000 })
  ...
  describe("Confidence Calibration", () => {
    it("should calibrate confidence scores", async () => {
      ...
      const confidence = await modelRouting.calculateConfidence(
        response,
        context
    ...
    it("should adjust confidence based on data quality", async () => {
      ...
      const highConfidence = await modelRouting.calculateConfidence(
        response,
        highQualityContext
      );
      const lowConfidence = await modelRouting.calculateConfidence(
        response,
        lowQualityContext
      );
  ...
  describe("Response Caching", () => {
    it("should cache responses appropriately", async () => {
      ...
      // First call
      const result1 = await modelRouting.routeRequest(
        "test prompt",
        context,
        {}

      // Second call should use cache
      const result2 = await modelRouting.routeRequest(
        "test prompt",
        context,
        {}
