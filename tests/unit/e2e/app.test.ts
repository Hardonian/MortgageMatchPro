...
test.describe("MortgageMatch Pro E2E Tests", () => {
  ...
  test("should display the main page with all components", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "MortgageMatch Pro" })
    ).toBeVisible();

    // Check if the tabs are visible
    await expect(
      page.getByRole("tab", { name: "Affordability" })
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Mortgage Affordability Calculator")
    ).toBeVisible();
  });
  ...
  test("should calculate affordability successfully", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="affordability-results"]', {
      timeout: 10000,
  ...
  test("should fetch and display rates", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="rate-comparison-table"]', {
      timeout: 10000,
  ...
  test("should compare scenarios", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="scenario-comparison"]', {
      timeout: 10000,
