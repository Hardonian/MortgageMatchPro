...
test.describe("Lead Generation Workflow", () => {
  ...
  test("should complete full lead generation flow", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="affordability-result"]')
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="lead-success-message"]')
    ).toBeVisible();
    await expect(page.locator('[data-testid="lead-score"]')).toContainText(
      "85"
    );
    await expect(
      page.locator('[data-testid="qualification-tier"]')
  ...
  test("should show validation errors for invalid input", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="credit-score-error"]')
    ).toContainText("between 300 and 850");
  });
  ...
  test("should require consent checkboxes", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="consent-error"]')).toContainText(
      "must consent"
    );
  });

  test("should display lead score and broker recommendations", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="broker-recommendations"]')
    ).toBeVisible();
