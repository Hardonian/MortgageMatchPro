import {
  ...
  trackSupabaseCost,
  trackStripeCost,
} from "../../../lib/optimization/cost-optimizer";
...
describe("CostOptimizer", () => {
  ...
  describe("Cost Tracking", () => {
    it("should track costs correctly", () => {
      optimizer.trackCost("openai", "chat_completion", 0.05, {
        model: "gpt-4",
      });
    ...
    it("should calculate monthly costs correctly", () => {
      ...
      optimizer.trackCost("openai", "chat_completion", 0.2, {
        timestamp: Date.now(),
      });
  ...
  describe("OpenAI Request Optimization", () => {
        ...
        messages: [
          {
            role: "user",
            content:
              "Please provide a detailed analysis of complex financial scenarios",
          },
        ],
      };
    ...
    it("should optimize prompts by removing redundant phrases", () => {
        ...
        messages: [
          {
            role: "system",
            content:
              "Please make sure to provide accurate information. It is important to note that you should be helpful.",
          },
          {
            role: "user",
      ...
      expect(optimized.messages[0].content).not.toContain(
        "Please make sure to"
      );
      expect(optimized.messages[0].content).not.toContain(
  ...
  describe("Optimization Recommendations", () => {
    it("should provide recommendations when budget is high", () => {
      ...
      expect(recommendations).toContain(
        "Daily budget is 80% used. Consider reducing request frequency."
      );
    });
    ...
    it("should provide recommendations for OpenAI-heavy usage", () => {
      ...
      expect(recommendations).toContain(
        "OpenAI costs are 80% of total. Consider implementing caching."
      );
    });
  });
...
describe("Cost Tracking Utilities", () => {
  ...
  it("should track Stripe costs", () => {
    trackStripeCost("payment", 0.3, { amount: 1000 });

    const dailyCost = costOptimizer.getDailyCost();
    expect(dailyCost).toBe(0.3);
  });
});
