...
describe("Lead Qualification Service", () => {
  ...
  describe("getQualificationTier", () => {
    ...
    it("should return STANDARD for medium scores", () => {
      expect(leadQualificationService.getQualificationTier(65)).toBe(
        "STANDARD"
      );
      expect(leadQualificationService.getQualificationTier(50)).toBe(
        "STANDARD"
      );
    });

    it("should return COACHING for low scores", () => {
      expect(leadQualificationService.getQualificationTier(45)).toBe(
        "COACHING"
      );
  ...
  describe("generateRoutingDecision", () => {
    it("should generate appropriate routing for premium tier", () => {
      const result = leadQualificationService.generateRoutingDecision(
        "PREMIUM",
        85
  ...
  describe("processLeadQualification", () => {
    it("should process complete lead qualification", async () => {
      ...
      const result = await leadQualificationService.processLeadQualification(
        mockLeadInput
      );
