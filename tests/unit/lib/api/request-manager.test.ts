...
describe("API Request Manager", () => {
  ...
  describe("apiRequests", () => {
    ...
    describe("Supabase requests", () => {
      ...
      it("should make insert request", async () => {
        ...
        const result = await apiRequests.supabase.insert("users", {
          name: "Test",
        });
      ...
      it("should make update request", async () => {
        ...
        const result = await apiRequests.supabase.update(
          "users",
          { name: "Updated" },
          "1"
        );
  ...
  describe("Utility functions", () => {
    it("should identify retryable errors", () => {
      expect(
        isRetryableError(new ApiError("Test", ApiErrorType.NETWORK_ERROR))
      ).toBe(true);
      expect(
        isRetryableError(new ApiError("Test", ApiErrorType.TIMEOUT_ERROR))
      ).toBe(true);
      expect(
        isRetryableError(new ApiError("Test", ApiErrorType.SERVER_ERROR))
      ).toBe(true);
      expect(
        isRetryableError(new ApiError("Test", ApiErrorType.VALIDATION_ERROR))
      ).toBe(false);
      expect(
        isRetryableError(
          new ApiError("Test", ApiErrorType.AUTHENTICATION_ERROR)
        )
      ).toBe(false);
    });
    ...
    it("should return correct error messages", () => {
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.NETWORK_ERROR))
      ).toBe(
        "Network connection failed. Please check your internet connection."
      );
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.TIMEOUT_ERROR))
      ).toBe("Request timed out. Please try again.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.VALIDATION_ERROR))
      ).toBe("Invalid data provided. Please check your input.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.AUTHENTICATION_ERROR))
      ).toBe("Authentication failed. Please log in again.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.AUTHORIZATION_ERROR))
      ).toBe("You do not have permission to perform this action.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.RATE_LIMIT_ERROR))
      ).toBe("Too many requests. Please wait a moment and try again.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.SERVER_ERROR))
      ).toBe("Server error occurred. Please try again later.");
      expect(
        getErrorMessage(new ApiError("Test", ApiErrorType.UNKNOWN_ERROR))
