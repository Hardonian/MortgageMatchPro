...
export class GlobalComplianceAgent {
  ...
  async mapTransactionToRequirements(transaction: {
    ...
    const jurisdictions = this.getApplicableJurisdictions(
    ...
    if (
      transaction.amount >
      this.reportingThresholds[transaction.sourceCountry] * 10
    ) {
      riskLevel = "critical";
    } else if (
      transaction.amount >
      this.reportingThresholds[transaction.sourceCountry] * 5
    ) {
      riskLevel = "high";
    } else if (
      transaction.amount > this.reportingThresholds[transaction.sourceCountry]
    ) {
      riskLevel = "medium";
    }
  ...
  async detectAnomalies(transaction: {
    ...
    const uniqueCountries = new Set(
      userHistory.map((t) => t.destinationCountry)
    );
  ...
  async generateAuditTrail(transaction: TransactionCompliance): Promise<{
    ...
    const auditTrailId = `AUDIT${Date.now()}${Math.random()
      .toString(36)
      .substr(2, 6)}`;
    ...
    const failedChecks = transaction.complianceChecks.filter(
      (c) => c.status === "failed"
    );
    const complianceStatus =
      failedChecks.length === 0 ? "compliant" : "non_compliant";

    // Calculate next review date (30 days for high-risk, 90 days for medium-risk, 1 year for low-risk)
    const reviewDays =
      transaction.riskScore > 70 ? 30 : transaction.riskScore > 40 ? 90 : 365;
  ...
  async generateBreachAlert(violation: ComplianceViolation): Promise<{
    ...
    const deadline = new Date(
      Date.now() + deadlineHours[severity] * 60 * 60 * 1000
    ).toISOString();
  ...
  async generateRegulatoryReport(
    ...
    await supabaseAdmin.from("regulatory_reports").insert([report]);

    return report;
  }

  // Helper methods
  private getApplicableJurisdictions(
    sourceCountry: string,
    destinationCountry: string
  ): string[] {
    ...
    for (const rule of rules) {
      if (rule.severity === "critical" || riskLevel === "critical") {
        recommendations.push(
          `CRITICAL: Implement ${rule.description} immediately`
        );
      } else if (rule.severity === "high" || riskLevel === "high") {
        recommendations.push(
          `HIGH: Address ${rule.description} within 24 hours`
        );
      } else if (rule.severity === "medium" || riskLevel === "medium") {
        recommendations.push(
          `MEDIUM: Review ${rule.description} within 7 days`
        );
      } else {
