...
export abstract class BaseAgent {
  ...
  protected async applyGuardrails(
    ...
    console.log(
      `[${this.config.name}] Guardrails applied for trace: ${this.traceId}`
    );
    return { allowed: true };
  }
...
Always include appropriate disclaimers and ensure all advice is compliance-verified.`,
      ...
      handoffs: ["broker_connection", "rate_verification", "compliance_review"],
    });
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "Detect the user's intent from their message. Return one of: calculate_affordability, fetch_rates, compare_scenarios, analyze_refinance, connect_broker, generate_amortization, or general_advice",
        },
        {
          role: "user",
  ...
  private async calculateAffordability(input: any): Promise<any> {
    ...
    return {
      ...
      recommendations: [
        ...
        "Your debt ratios are within acceptable limits",
      ],
      disclaimers: [
