...
export class ScenarioSimulator {
  ...
  async runStressTestSimulation(
    ...
    try {
      ...
      await this.storeSimulationResults(
        simulationId,
        userId,
        "stress_test",
        results,
  ...
  private async runSingleSimulation(
    ...
    let currentBalance =
      baseScenario.parameters.propertyPrice -
      baseScenario.parameters.downPayment;
    ...
    for (let month = 0; month < timeHorizon; month++) {
      ...
      const monthlyPayment = this.calculateMonthlyPayment(
        currentBalance,
        currentRate,
        baseScenario.parameters.termYears
      );
      ...
      const principalPayment =
        monthlyPayment - (currentBalance * currentRate) / 100 / 12;
    ...
    const refinanceProbability = this.calculateRefinanceProbability(
      currentRate,
      baseScenario.parameters.interestRate
    );
  ...
  private calculateDefaultProbability(
    ...
    for (let i = 0; i < monthlyPayments.length; i++) {
      ...
      if (paymentToIncomeRatio > 0.4) {
        // DTI > 40% indicates high default risk
        defaultCount++;
      }
    }
  ...
  private calculateRefinanceProbability(
    currentRate: number,
    originalRate: number
  ): number {
  ...
  private calculateSummaryStatistics(
    ...
    return {
      ...
      results: {
        ...
        defaultRate:
          allDefaultRates.reduce((a, b) => a + b, 0) / allDefaultRates.length,
        refinanceRate:
          allRefinanceRates.reduce((a, b) => a + b, 0) /
          allRefinanceRates.length,
      },
      stressTestResults: {
        rateShock: {
  ...
  private calculateRateShockImpact(
    results: SimulationResult[],
    shockBps: number
  ): number {
  ...
  private calculatePropertyDeclineImpact(
    results: SimulationResult[],
  ...
  private calculateIncomeReductionImpact(
    results: SimulationResult[],
  ...
  async runComprehensiveStressTest(
    ...
    try {
      ...
      results.push(
        await this.runStressTestSimulation(userId, stressTestParams)
      );
      ...
      results.push(
        await this.runRateShockSimulation(userId, baseScenario, rateShocks)
      );

      // Property decline scenarios
      const propertyDeclines = [
        { name: "Mild Decline", declinePercent: -0.05 },
        { name: "Moderate Decline", declinePercent: -0.1 },
        { name: "Severe Decline", declinePercent: -0.2 },
      ];
      results.push(
        await this.runPropertyDeclineSimulation(
          userId,
          baseScenario,
          propertyDeclines
        )
      ...
      const incomeVariances = [
        { name: "Income Reduction", incomeChange: -0.1 },
        { name: "Severe Income Reduction", incomeChange: -0.2 },
        { name: "Income Growth", incomeChange: 0.1 },
      ];
      results.push(
        await this.runIncomeVarianceSimulation(
          userId,
          baseScenario,
          incomeVariances
        )
