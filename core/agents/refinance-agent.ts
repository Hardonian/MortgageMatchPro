...
export class RefinanceAgent {
    ...
    try {
      ...
      for (const user of users || []) {
        try {
          ...
          const forecast =
            await this.forecastingAgent.predictRefinanceProbability(
              user.id,
          ...
          if (forecast.probability > 0.3) {
            const watchlistItem: RefinanceWatchlistItem = {
              ...
              nextContactDate: this.calculateNextContactDate(
  ...
  async getRefinanceWatchlist(
    userId?: string
  ): Promise<RefinanceWatchlistItem[]> {
    try {
      ...
      return (data || []).map((row) => ({
        ...
        recommendedAction: this.getRecommendedAction(
          row.refinance_probability,
          row.potential_savings
        ),
        nextContactDate: row.last_contacted
          ? new Date(
              new Date(row.last_contacted).getTime() + 7 * 24 * 60 * 60 * 1000
            ).toISOString()
  ...
  async triggerRetainBotContact(
    watchlistItem: RefinanceWatchlistItem
  ): Promise<void> {
    try {
      ...
      await supabaseAdmin.from("prediction_alerts").insert({
      ...
      console.log(
        `RetainBot message sent to user ${watchlistItem.userId}: ${message}`
      );
    } catch (error) {
  ...
  private async generateRetainBotMessage(
    item: RefinanceWatchlistItem
  ): Promise<string> {
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are RetainBot, an AI assistant that helps mortgage brokers retain clients. Generate a personalized message about refinancing opportunities. Be friendly, professional, and specific about potential savings.",
          },
          {
            role: "user",
      ...
      return (
        ...
        "Refinancing could save you money. Contact us for details."
      );
    } catch (error) {
      console.error("Error generating RetainBot message:", error);
      return `Hi! Based on current market conditions, refinancing could save you $${item.potentialSavings.toLocaleString()}. Your current rate of ${
        item.currentRate
      }% is higher than current market rates. ${
  ...
  private async storeWatchlistItems(
    items: RefinanceWatchlistItem[]
  ): Promise<void> {
    try {
      const { error } = await supabaseAdmin.from("refinance_watchlist").upsert(
        items.map((item) => ({
          id: item.id,
          user_id: item.userId,
      ...
      for (const item of watchlistItems || []) {
        if (item.current_rate - currentMarketRate > 0.75) {
          await supabaseAdmin.from("prediction_alerts").insert({
            ...
            message: `Rates have dropped! Current market rate is ${currentMarketRate.toFixed(
              3
            )}%, your rate is ${
              item.current_rate
            }%. Refinancing could save you $${item.potential_savings.toLocaleString()}.`,
          });
        }
      }
    } catch (error) {
