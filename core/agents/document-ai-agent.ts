...
export class DocumentAIAgent {
  ...
  async processDocument(
    ...
    try {
      console.log(
        `Processing document ${documentId} for user ${upload.userId}`
      );
      ...
      const documentType = await this.classifyDocumentType(
        extractedText,
        upload.mimeType
      );
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a financial document extraction expert. Extract structured data from financial documents with high accuracy. Return valid JSON matching the ExtractedDataSchema.",
          },
          {
            role: "user",
            content: prompt,
  ...
  private async getRelevantContext(
    ...
    try {
      ...
      return similarDocs.map((doc) => doc.extraction_examples).join("\n\n");
    } catch (error) {
  ...
  private getDefaultContext(documentType: DocumentType): string {
    const contexts = {
      tax_return:
        "Extract: Total income, taxable income, deductions, refunds, filing status, dependents, employment income, investment income, business income.",
      paystub:
        "Extract: Gross pay, net pay, pay period, year-to-date amounts, deductions (taxes, insurance, retirement), employer name, employee ID.",
      ...
      bank_statement:
        "Extract: Account number, statement period, beginning balance, ending balance, total deposits, total withdrawals, account type, bank name.",
      employment_letter:
        "Extract: Employee name, position, start date, salary, employment status, employer name, letter date, manager signature.",
      credit_report:
        "Extract: Credit score, credit utilization, payment history, account balances, credit limits, inquiries, derogatory marks, account types.",
      property_assessment:
        "Extract: Property address, assessed value, property type, lot size, building size, year built, tax assessment, market value estimate.",
      insurance_document:
        "Extract: Policy number, coverage amount, premium, deductible, policy period, coverage type, insurer name, beneficiary.",
      other:
        "Extract any relevant financial information, dates, amounts, names, and identifying information.",
    };
  ...
  private async verifyExtractedData(
    ...
    try {
      ...
      return {
        ...extractedData,
        verificationFlags: [
          ...extractedData.verificationFlags,
          ...verificationFlags,
        ],
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a mortgage underwriting expert. Generate a comprehensive affordability report based on extracted financial document data. Include verification status, risk factors, and recommendations.",
          },
          {
            role: "user",
