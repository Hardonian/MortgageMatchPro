...
export type MacroEconomicIndicator = z.infer<
  typeof MacroEconomicIndicatorSchema
>;
...
export class TreasuryRiskAgent {
  ...
  constructor() {
    ...
    this.baseURL =
      process.env.TREASURY_API_BASE_URL || "https://api.treasury.com/v1";
  ...
  async calculateFXRisk(
    ...
    try {
      ...
      const hedgingStrategy = await this.getCurrentHedgingStrategy(
        currencyPair

      // Calculate hedge ratio and cost
      const hedgeRatio = hedgingStrategy !== "none" ? 0.8 : 0; // 80% hedge ratio
      const costOfHedge = this.calculateHedgeCost(
        exposure,
        currencyPair,
        hedgingStrategy
      );
    ...
    for (const position of positions) {
      try {
        ...
        const hedgeAmount = this.calculateHedgeAmount(
          exposureUSD,
          recommendation.hedgeRatio
        );
        const hedgeRate = this.getHedgeRate(
          position.currency,
          recommendation.hedgeType
        );
        const cost = this.calculateHedgeCost(
          hedgeAmount,
          position.currency,
          recommendation.hedgeType
        );
        ...
        const hedgingRecommendation: HedgingRecommendation = {
          recommendationId: `HEDGE-${Date.now()}-${Math.random()
            .toString(36)
            .substr(2, 6)}`,
        ...
        console.error(
          `Error generating hedging recommendation for position ${position.positionId}:`,
          error
        );
      }
    }
  ...
  async generateFXForecast(
    ...
    try {
      ...
      const forecasts = this.generateEconomicForecast(
        currentRate,
        relevantIndicators,
        horizonDays
      ...
      throw new Error(
        `Failed to generate FX forecast: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  async monitorTreasuryPositions(): Promise<{
    ...
    for (const [positionId, position] of this.positions) {
      ...
      const positionVaR = this.calculateValueAtRisk(
        position.valueUSD,
        volatility,
        30
      );
    ...
    if (marketVolatility > 0.3) {
      alerts.push({
        ...
        recommendedAction:
          "Review all positions and consider additional hedging",
      });
    }

    const averageRiskScore =
      this.positions.size > 0 ? totalRiskScore / this.positions.size : 0;
  ...
  private async getHistoricalVolatility(
    currencyPair: string,
    days: number
  ): Promise<number> {
    try {
      ...
      return mockVolatilities[currencyPair] || 0.1;
    }
  }
  ...
  private async getCurrentHedgingStrategy(
    currencyPair: string
  ): Promise<"none" | "forward" | "option" | "swap" | "natural"> {
  ...
  private async mlHedgingDecision(
    ...
    if (volatility > 0.2 || position.riskScore > 70) {
      ...
      confidence = 0.8;
      reasoning =
        "High volatility and risk score warrant full hedging with options";
  ...
  private calculateExpectedBenefit(
    ...
    const expectedRateChange =
      forecast.forecasts[0]?.rate - forecast.currentRate || 0;
    return (
      hedgeAmount * Math.abs(expectedRateChange) * recommendation.confidence;
    );
  }
  ...
  private getRelevantIndicators(currency: string): MacroEconomicIndicator[] {
    ...
    const indicatorIds = currencyMap[currency] || [
      "US-GDP",
      "US-INTEREST-RATE",
    ];
    return indicatorIds
      .map((id) => this.economicIndicators.get(id))
      .filter(Boolean) as MacroEconomicIndicator[];
  }
  ...
  private getCurrencyVolatility(currency: string): number {
    ...
    return volatilities[currency] || 0.1;
  }
}
