...
export class EnhancedExplainabilityAgent {
  ...
  async generateSHAPExplanation(
    ...
    try {
      console.log(
        `Generating SHAP explanation for prediction ${request.predictionId}`
      );
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Explain this SHAP analysis for a ${
              request.modelType
            } prediction:
...
Return JSON matching SHAPExplanationSchema.`,
  ...
  async generateLIMEExplanation(
    ...
    try {
      console.log(
        `Generating LIME explanation for prediction ${request.predictionId}`
      );

      // Simulate LIME calculation (in production, use actual LIME library)
      const limeValues = await this.calculateLIMEValues(
        request.inputData,
        request.modelType
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Explain this LIME analysis for a ${
              request.modelType
            } prediction:
  ...
  private async generateCombinedExplanation(
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Combine these explanations for a ${
            request.modelType
          } prediction:
