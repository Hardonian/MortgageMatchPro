...
export class ForecastingAgent {
    ...
    try {
      ...
      for (let i = 1; i <= forecastMonths; i++) {
        ...
        const forecast: RateForecast = {
          ...
          confidenceIntervalLower: Math.max(
            0,
          ...
          confidenceScore: Math.max(
            0,
            Math.min(1, 1 - (volatility * i) / currentRate)
          ),
          modelVersion: "v1.0.0",
          inputData: {
          ...
          rateChangePercent:
            ((predictedValue - currentRate) / currentRate) * 100,
        };
    ...
    try {
      ...
      for (let i = 1; i <= forecastMonths; i++) {
        ...
        const confidenceInterval =
          currentValue * volatility * Math.sqrt(i) * 1.96;
  ...
  async predictRefinanceProbability(
    ...
    try {
      ...
      const userData = await this.dataIngestionAgent.ingestUserPortfolioData(
        userId

      // Get current market rates
      const currentMarketRates =
        await this.dataIngestionAgent.getHistoricalData(
          "mortgage_rates",
          "CA",
      ...
      const currentMarketRate =
        currentMarketRates[currentMarketRates.length - 1]?.value || currentRate;
