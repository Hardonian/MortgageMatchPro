...
export class WelcomeFlowAgent {
  ...
  async generateContextualHelp(
    ...
    try {
      ...
      await supabaseAdmin.from("personalized_content").insert({
        user_id: this.userId,
  ...
  async detectDropOff(): Promise<{
    ...
    try {
      ...
      const timeSinceLastActivity =
        Date.now() - new Date(flowData.last_activity_at).getTime();
  ...
  private async determineFlowType(): Promise<string> {
    ...
    try {
      ...
      return (
        completion.choices[0].message.content?.trim() || "first_time_homebuyer"
      );
    } catch (error) {
  ...
  private async generatePersonalizedSteps(
    flowType: string
    ...
    return (
      baseSteps[flowType as keyof typeof baseSteps] ||
      baseSteps.first_time_homebuyer
    );
  }
  ...
  private async updateStepCompletion(
    stepId: string,
    response: any
  ): Promise<void> {
    const { error } = await supabaseAdmin
      .from("user_onboarding_flows")
      .update({
        completed_steps: supabaseAdmin.raw(
          `completed_steps || '["${stepId}"]'::jsonb`
        ),
  ...
  private async updateUserPreferences(
    ...
    await supabaseAdmin
      .from("user_onboarding_flows")
      .update({
        preferences: supabaseAdmin.raw(
          `preferences || '${JSON.stringify(data)}'::jsonb`
        ),
      })
      .eq("user_id", this.userId);
  }
