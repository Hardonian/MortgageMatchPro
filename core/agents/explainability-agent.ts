...
export class ExplainabilityAgent {
  ...
  async generateSHAPExplanation(
    ...
    try {
      ...
      const featureContributions = this.calculateSHAPValues(
        inputFeatures,
        modelType
      );
      const globalImportance =
        this.calculateGlobalImportance(featureContributions);
      ...
      const explanation: ModelExplanation = {
        ...
        globalImportance,
        confidence: 0.85,
        interpretation: await this.generateInterpretation(
          featureContributions,
          modelType
        ),
  ...
  async generateLIMEExplanation(
    ...
    try {
      ...
      const featureContributions = this.calculateLIMEValues(
        inputFeatures,
        modelType
      );
      const globalImportance =
  ...
  async generateFeatureImportanceExplanation(
    ...
    try {
      const featureContributions = this.calculateFeatureImportance(
        inputFeatures,
        modelType
      );
      const globalImportance =
  ...
  private calculateFeatureImportance(
    ...
    for (const feature of Object.keys(inputFeatures)) {
      importance[feature] =
        featureWeights[feature as keyof typeof featureWeights] || 0.01;
    }
  ...
  private calculateGlobalImportance(
    ...
    for (const [feature, contribution] of Object.entries(contributions)) {
      globalImportance[feature] =
        total > 0 ? Math.abs(contribution) / total : 0;
    }
...
Provide a clear explanation of how each factor influences the prediction, focusing on the most important factors.`,
  ...
  private generateComplianceNotes(
    ...
    if (
      contributions.credit_score &&
      Math.abs(contributions.credit_score) > 0.3
    ) {
      notes.push(
        "Credit score has significant impact - ensure fair lending compliance"
      );

    if (contributions.income && Math.abs(contributions.income) > 0.2) {
      notes.push(
        "Income factor is prominent - verify non-discriminatory practices"
      );
    ...
    for (const feature of protectedFeatures) {
      if (contributions[feature] && Math.abs(contributions[feature]) > 0.1) {
        notes.push(
          `Protected characteristic '${feature}' detected - review for compliance`
        );
      }
  ...
  private calculateFairnessMetrics(
    testData: any[]
  ): ComplianceReport["fairnessMetrics"] {
    return {
  ...
  private assessRegulatoryCompliance(
    testData: any[]
  ): ComplianceReport["regulatoryCompliance"] {
    const issues: string[] = [];
  ...
  private async storeExplanation(explanation: ModelExplanation): Promise<void> {
    try {
      const { error } = await supabaseAdmin.from("model_explanations").insert({
        id: explanation.id,
  ...
  private async storeComplianceReport(report: ComplianceReport): Promise<void> {
    try {
      ...
      for (const metric of metrics) {
        await supabaseAdmin.from("model_metrics").insert({
