...
export class PrescriptiveAgent {
  ...
  async analyzeWhatIfScenarios(
    ...
    try {
      ...
      for (const modification of modifications) {
        ...
        modifiedScenario.parameters[modification.parameter] =
          modification.newValue;

        // Calculate new scenario results
        const newResults = await this.calculateScenarioResults(
          modifiedScenario
        );
        ...
        analysis.modifications.push({
          ...
          impact: {
            monthlyPaymentChange:
              newResults.monthlyPayment - baseResults.monthlyPayment,
            totalInterestChange:
              newResults.totalInterest - baseResults.totalInterest,
            qualificationImpact:
              newResults.qualificationResult !==
              baseResults.qualificationResult,
          },
        });
      }
  ...
  async generateOptimalRecommendations(
    userId: string
    ...
    try {
      ...
      const rateForecasts = await this.forecastingAgent.getLatestForecasts(
        userId,
        "rate_forecast"
      );
      const propertyForecasts = await this.forecastingAgent.getLatestForecasts(
        userId,
      ...
      const extraPaymentRec = await this.analyzeExtraPayments(
        calculation,
        rateForecasts
      );
      if (extraPaymentRec) {
      ...
      const termShorteningRec = await this.analyzeTermShortening(
        calculation,
        rateForecasts
      );
      ...
      const rateSwitchRec = await this.analyzeRateSwitch(
        calculation,
        rateForecasts
      );
      if (rateSwitchRec) {
      ...
      const refinanceRec = await this.analyzeRefinancing(
        calculation,
        rateForecasts
      );
      if (refinanceRec) {
      ...
      const propertyRec = await this.analyzePropertyOptimization(
        calculation,
        propertyForecasts
      );
      if (propertyRec) {
  ...
  private async analyzeExtraPayments(
    ...
    try {
      ...
      const newMonthlyPayment =
        calculation.monthly_payment + extraPaymentAmount;
      ...
      if (totalInterestSaved > 5000) {
        // Only recommend if savings > $5,000
        return {
          ...
          description:
            "Making extra principal payments reduces total interest and shortens loan term",
          expectedOutcome: {
          ...
          confidence: 0.85,
          reasoning: `Based on your current rate of ${
            calculation.interest_rate
          }%, extra payments of $${extraPaymentAmount.toFixed(
            0
          )} monthly will save $${totalInterestSaved.toLocaleString()} in interest over the life of the loan.`,
          implementationSteps: [
          ...
          priority:
            totalInterestSaved > 15000
              ? "high"
              : totalInterestSaved > 8000
              ? "medium"
              : "low",
  ...
  private async analyzeTermShortening(
    ...
    try {
      ...
      const totalInterestSaved =
        calculation.total_interest -
        newSchedule[newSchedule.length - 1].cumulativeInterest;
      const monthlyPaymentIncrease =
        newMonthlyPayment - calculation.monthly_payment;

      if (
        totalInterestSaved > 10000 &&
        monthlyPaymentIncrease < calculation.monthly_payment * 0.3
      ) {
        return {
          ...
          description:
            "Shortening the loan term reduces total interest paid and builds equity faster",
          expectedOutcome: {
          ...
          confidence: 0.8,
          reasoning: `Reducing your term to ${newTerm} years will increase your monthly payment by $${monthlyPaymentIncrease.toFixed(
            0
          )} but save $${totalInterestSaved.toLocaleString()} in total interest.`,
          implementationSteps: [
  ...
  private async analyzeRateSwitch(
    ...
    try {
      if (calculation.rate_type === "fixed") {
        return null;
      }
      ...
      if (forecastedRates.length === 0) {
        return null;
      }

      const averageForecastedRate =
        forecastedRates.reduce((sum, f) => sum + f.predictedValue, 0) /
        forecastedRates.length;
      const rateIncrease = averageForecastedRate - currentRate;

      if (rateIncrease > 0.5) {
        // If rates expected to increase by >0.5%
        ...
        return {
          ...
          expectedOutcome: {
            monthlyPaymentChange: monthlyPaymentIncrease,
            totalInterestChange:
              monthlyPaymentIncrease * calculation.term_years * 12,
            totalCostChange:
              monthlyPaymentIncrease * calculation.term_years * 12,
            paybackPeriod: 0,
          },
          confidence: 0.75,
          reasoning: `Rates are forecasted to increase by ${rateIncrease.toFixed(
            2
          )}% over the next year. Switching to a fixed rate now will provide payment stability and protect against future increases.`,
          implementationSteps: [
  ...
  private async analyzeRefinancing(
    ...
    try {
      ...
      if (monthlySavings > 100 && paybackPeriod < 3) {
        // Savings > $100/month, payback < 3 years
        return {
          ...
          description:
            "Refinancing to a lower rate reduces monthly payments and total interest",
          expectedOutcome: {
          ...
          confidence: 0.7,
          reasoning: `Refinancing to ${marketRate}% will save $${monthlySavings.toFixed(
            0
          )} monthly and $${annualSavings.toFixed(
            0
          )} annually. The $${refinancingCost.toLocaleString()} cost will be recovered in ${paybackPeriod.toFixed(
            1
          )} years.`,
          implementationSteps: [
  ...
  private async analyzePropertyOptimization(
    ...
    try {
      ...
      if (appreciationRate > 0.05) {
        // 5% appreciation expected
        return {
          ...
          description:
            "Strategic improvements can increase property value and equity",
          expectedOutcome: {
          ...
          confidence: 0.6,
          reasoning: `Your property is expected to appreciate by ${(
            appreciationRate * 100
          ).toFixed(
            1
          )}%. Strategic improvements could further increase this appreciation and build more equity.`,
          implementationSteps: [
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a financial advisor analyzing mortgage scenarios. Generate specific, actionable recommendations based on the scenario analysis. Be practical and consider the user's financial situation.",
          },
          {
            role: "user",
...
Provide 2-3 specific recommendations with clear reasoning and implementation steps.`,
      ...
      const recommendations: PrescriptiveRecommendation[] = [
        {
          ...
          risks: [
            "AI recommendations may not fit personal situation",
  ...
  private async calculateScenarioResults(scenario: any): Promise<any> {
    ...
    const principal =
      scenario.parameters.propertyPrice - scenario.parameters.downPayment;
