...
export class DataIngestionAgent {
  private readonly BANK_OF_CANADA_API =
    "https://www.bankofcanada.ca/valet/observations";
  private readonly STATS_CANADA_API =
    "https://www150.statcan.gc.ca/t1/wds/rest/getBulkData";
  private readonly CREA_API = "https://crea.ca/api/statistics";

  // Ingest mortgage rate data from Bank of Canada
  async ingestMortgageRates(region = "CA"): Promise<MarketDataPoint[]> {
    try {
  ...
  async ingestIncomeTrends(region = "CA"): Promise<IncomeTrendsData[]> {
    try {
  ...
  async ingestMarketIndices(region = "CA"): Promise<MarketIndexData[]> {
    try {
