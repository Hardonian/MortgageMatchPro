...
export class DataRetentionService {
  ...
  private async processRetentionRule(rule: DataRetentionRule): Promise<void> {
    ...
    console.log(
      `   Processing ${recordsToProcess.length} records for rule: ${rule.id}`
    );
  ...
  private async anonymizeRecords(
    ...
    console.log(
      `   Anonymizing ${records.length} records for rule: ${rule.id}`
    );
   ...
   */
  async addRetentionRule(
    rule: Omit<DataRetentionRule, "id">
  ...
  async updateRetentionRule(
    ...
    const ruleIndex = this.retentionRules.findIndex(
      (rule) => rule.id === ruleId
  ...
  async getRetentionStatistics(): Promise<{
    ...
    return {
      ...
      recordsProcessedToday: todayLogs.reduce(
        (sum, log) => sum + log.recordCount,
        0
      ),
      recordsProcessedThisWeek: weekLogs.reduce(
        (sum, log) => sum + log.recordCount,
        0
      ),
      recordsProcessedThisMonth: monthLogs.reduce(
        (sum, log) => sum + log.recordCount,
        0
      ),
      topDataTypes,
