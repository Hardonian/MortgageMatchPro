...
export class ResourceManager extends EventEmitter {
  ...
  allocateResources(
    ...
    // Create allocation
    const allocationId = `${type}-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  releaseResources(allocationId: string): boolean {
    ...
    // Update resource capacity
    config.currentCapacity = Math.max(
      0,
      config.currentCapacity - allocation.amount
    );
    ...
    for (const [type, usage] of this.usage.entries()) {
      ...
      if (usage.status === ResourceStatus.OVERLOADED) {
        recommendations.push({
          type,
          recommendation: `Resource ${type} is overloaded (${usage.percentage.toFixed(
            1
          )}%). Consider scaling up or optimizing usage.`,
          priority: "high",
        ...
        recommendations.push({
          type,
          recommendation: `Resource ${type} is underutilized (${usage.percentage.toFixed(
            1
          )}%). Consider scaling down to reduce costs.`,
          priority: "medium",
          potentialSavings:
            config.cost * (config.maxCapacity - usage.used) * 0.5,
        });
      }
    }
  ...
  getStatistics(): {
    ...
    const totalCost = Array.from(this.allocations.values()).reduce(
      (sum, allocation) => sum + allocation.cost,
      0
    );
    const averageUtilization =
      Array.from(this.usage.values()).reduce(
        (sum, usage) => sum + usage.percentage,
        0
      ) / this.usage.size;
    const overloadedResources = Array.from(this.usage.values()).filter(
      (usage) => usage.status === ResourceStatus.OVERLOADED
    ).length;
