...
export class LeadMonitoringService {
  ...
  async getLeadMetrics(): Promise<LeadMetrics> {
    try {
      ...
      const leadsToday =
        leads?.filter((lead) => new Date(lead.created_at) >= today).length || 0;
      const leadsThisWeek =
        leads?.filter((lead) => new Date(lead.created_at) >= weekAgo).length ||
        0;
      const leadsThisMonth =
        leads?.filter((lead) => new Date(lead.created_at) >= monthAgo).length ||
        0;
      ...
      const leadsByStatus = {
        ...
        rejected:
          leads?.filter((lead) => lead.status === "rejected").length || 0,
      };
      ...
      const leadsByTier = {
        ...
        standard:
          leads?.filter((lead) => lead.lead_score >= 50 && lead.lead_score < 70)
            .length || 0,
      ...
      const brokerPerformance =
        brokers
          ?.map((broker) => {
            const brokerLeads =
              leads?.filter((lead) => lead.broker_id === broker.id) || [];
            const convertedBrokerLeads = brokerLeads.filter(
  ...
  > {
    ...
    if (metrics.conversionRate < 10 && metrics.totalLeads > 50) {
      alerts.push({
        type: "low_conversion",
        severity: "high",
        message: `Low conversion rate: ${metrics.conversionRate.toFixed(
          1
        )}% (expected >10%)`,
        timestamp: now,
        resolved: false,
    ...
    const inactiveBrokers =
      brokers?.filter((broker) => {
        const brokerLeads =
          leads?.filter((lead) => lead.broker_id === broker.id) || [];
        const recentLeads = brokerLeads.filter(
          (lead) =>
            new Date(lead.created_at) >
            new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
        );
        return (
          broker.is_active && recentLeads.length === 0 && brokerLeads.length > 0;
        );
      }) || [];
    ...
    if (metrics.averageLeadScore < 40 && metrics.totalLeads > 20) {
      alerts.push({
        type: "low_conversion",
        severity: "medium",
        message: `Low average lead score: ${metrics.averageLeadScore.toFixed(
          1
        )}/100`,
        timestamp: now,
        resolved: false,
