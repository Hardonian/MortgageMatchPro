...
export class LeadQualificationService {
  ...
  calculateLeadScore(input: LeadQualificationInput): number {
    ...
    if (
      downPaymentPercentage >= LEAD_SCORING_CRITERIA.DOWN_PAYMENT_PERCENTAGE
    ) {
    ...
    const monthlyDebts = this.estimateMonthlyDebts(
      input.income,
      input.propertyValue,
      input.downPayment
    );
  ...
  getQualificationTier(score: number): keyof typeof LEAD_ROUTING_TIERS {
    if (score >= LEAD_ROUTING_TIERS.PREMIUM.minScore) {
      return "PREMIUM";
    }
    if (score >= LEAD_ROUTING_TIERS.STANDARD.minScore) {
      return "STANDARD";
    }
    return "COACHING";
  }
    ...
    if (preferredLender) {
      filteredBrokers = brokers.filter(
        (broker) =>
          broker.company
            .toLowerCase()
            .includes(preferredLender.toLowerCase()) ||
  ...
  async processLeadQualification(
    ...
    const { routingDecision, nextSteps, disclaimers } =
      this.generateRoutingDecision(qualificationTier, leadScore);
