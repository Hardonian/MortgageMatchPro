...
export class AIPricingService {
  ...
  private async predictWithModel(
    ...
    switch (model.id) {
      case "volume-based":
        adjustedPrice = this.calculateVolumeBasedPricing(
          params,
          factors,
          reasoning
        );
      ...
      case "latency-optimized":
        adjustedPrice = this.calculateLatencyOptimizedPricing(
          params,
          factors,
          reasoning
        );
      ...
      case "market-adaptive":
        adjustedPrice = this.calculateMarketAdaptivePricing(
          params,
          factors,
          reasoning
        );
  ...
  private calculateVolumeBasedPricing(
    ...
    if (factors.volume >= params.volumeThreshold3) {
      ...
      reasoning.push(
        `Volume discount applied: ${params.volumeDiscount3 * 100}% (${
          factors.volume
        } calls)`
      );
      ...
      reasoning.push(
        `Volume discount applied: ${params.volumeDiscount2 * 100}% (${
          factors.volume
        } calls)`
      );
      ...
      reasoning.push(
        `Volume discount applied: ${params.volumeDiscount1 * 100}% (${
          factors.volume
        } calls)`
      );
    }
  ...
  private calculateLatencyOptimizedPricing(
    params: Record<string, number>,
    factors: PricingFactors,
    reasoning: string[]
  ): number {
    ...
    if (factors.latency > params.latencyThreshold) {
      ...
      reasoning.push(
        `Latency penalty applied: ${params.latencyPenalty * 100}% (${
          factors.latency
        }ms)`
      );
      ...
      reasoning.push(
        `Latency bonus applied: ${params.latencyBonus * 100}% (${
          factors.latency
        }ms)`
      );
    }
  ...
  private calculateMarketAdaptivePricing(
    params: Record<string, number>,
    factors: PricingFactors,
    reasoning: string[]
  ): number {
    ...
    if (factors.marketVolatility > params.volatilityThreshold) {
      const volatilityAdjustment =
        (factors.marketVolatility - params.volatilityThreshold) *
        params.volatilityMultiplier;
      price *= 1 + volatilityAdjustment;
      reasoning.push(
        `Market volatility adjustment: ${volatilityAdjustment * 100}%`
      );
    }

    // Competition adjustment
    const competitionAdjustment =
      factors.competition * params.competitionWeight;
  ...
  private calculateFactorMultipliers(factors: PricingFactors) {
    return {
      ...
      seasonalityMultiplier: this.calculateSeasonalityMultiplier(
        factors.seasonality
      ),
      tierMultiplier: this.calculateTierMultiplier(factors.userTier),
      competitionMultiplier: this.calculateCompetitionMultiplier(
        factors.competition
      ),
    };
  }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Pricing experiment creation failed",
      };
    }
  }
    ...
    try {
      ...
      if (usage && usage.length > 0) {
        ...
        if (avgCallsPerDay > 50) {
          recommendations.push({
            ...
            description: `You're using ${Math.round(
              avgCallsPerDay
            )} calls per day. Upgrading to Pro tier could save you money.`,
        ...
        if (totalCalls > 1000) {
          recommendations.push({
            ...
            description:
              "Consider batching API calls during off-peak hours to reduce costs.",
        ...
        if (peakHourCalls / totalCalls > 0.8) {
          recommendations.push({
            ...
            description:
              "Most of your calls are during peak hours. Shifting to off-peak could save 5-10%.",
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Get pricing recommendations failed",
      };
    }
  }
