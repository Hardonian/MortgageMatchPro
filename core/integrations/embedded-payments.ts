...
export class EmbeddedPaymentService {
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Secure holdback creation failed",
      };
    }
  }
  ...
  async releaseHoldback(
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Holdback release failed",
      };
    }
  }
  ...
  async calculateRevenueShare(data: {
    ...
    try {
      ...
      const shareAmount =
        (data.amount * partner.revenue_share_percentage) / 100;
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Revenue share calculation failed",
      };
    }
  }
  ...
  async storePayment(
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Payment storage failed",
      };
    }
  }
  ...
  async getPaymentHistory(
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get payment history failed",
      };
    }
  }
  ...
  calculateFees(
    ...
    const feeStructure = {
      commission: { base: 0.3, percentage: 0.029 },
      referral: { base: 0.25, percentage: 0.025 },
      api_usage: { base: 0.1, percentage: 0.015 },
      subscription: { base: 0.5, percentage: 0.035 },
      broker_fee: { base: 1.0, percentage: 0.045 },
    };

    const fees =
      feeStructure[paymentType as keyof typeof feeStructure] ||
      feeStructure.api_usage;
    ...
    const adjustedPercentage = Math.max(
      0.005,
      fees.percentage - volumeDiscount
    );
...
export function createEmbeddedPaymentService(
  provider: PaymentProvider
