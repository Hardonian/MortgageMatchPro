...
export class DummyBankAdapter {
  ...
  private integrationId = "dummybank-api";
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      (config) => {
        console.log(
          `DummyBank API Request: ${config.method?.toUpperCase()} ${config.url}`
        );
        return config;
      },
      (error) => {
    ...
    this.client.interceptors.response.use(
      (response) => {
        console.log(
          `DummyBank API Response: ${response.status} ${response.config.url}`
        );
        return response;
      },
      (error) => {
        console.error(
          "DummyBank API Response Error:",
