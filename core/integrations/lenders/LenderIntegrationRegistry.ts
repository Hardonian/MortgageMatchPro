...
export class LenderIntegrationRegistry {
  ...
  getActiveIntegrations(): IntegrationStatus[] {
    return this.registry.integrations.filter(
  ...
  getIntegration(id: string): IntegrationStatus | undefined {
    return this.registry.integrations.find(
  ...
  async registerIntegration(
    integration: Omit<IntegrationStatus, "lastChecked" | "health">
  ): Promise<void> {
    try {
      ...
      const existingIndex = this.registry.integrations.findIndex(
  ...
  async checkIntegrationHealth(id: string): Promise<{
    ...
    try {
      ...
      if (integration.health.uptime < 0.95) {
        issues.push(
          `Uptime is ${(integration.health.uptime * 100).toFixed(
            1
          )}% (below 95%)`
        );
      ...
      if (integration.health.responseTime > 5000) {
        issues.push(
          `Response time is ${integration.health.responseTime}ms (above 5s)`
        );
      ...
      if (integration.health.errorRate > 0.05) {
        issues.push(
          `Error rate is ${(integration.health.errorRate * 100).toFixed(
            1
          )}% (above 5%)`
        );
      ...
      // Check rate limits
      if (
        integration.rateLimit.currentUsage >
        integration.rateLimit.requestsPerMinute * 0.9
      ) {
  ...
  getIntegrationStats(): {
    ...
    const maintenance = integrations.filter(
      (i) => i.status === "maintenance"
    ).length;
    const error = integrations.filter((i) => i.status === "error").length;

    const averageUptime =
      integrations.reduce((sum, i) => sum + i.health.uptime, 0) / total;
    const averageResponseTime =
      integrations.reduce((sum, i) => sum + i.health.responseTime, 0) / total;
    const averageErrorRate =
      integrations.reduce((sum, i) => sum + i.health.errorRate, 0) / total;
...
export const lenderIntegrationRegistry =
  LenderIntegrationRegistry.getInstance();

// Convenience functions
export const getIntegrations = () =>
  lenderIntegrationRegistry.getIntegrations();
export const getActiveIntegrations = () =>
  lenderIntegrationRegistry.getActiveIntegrations();
export const getIntegration = (id: string) =>
  lenderIntegrationRegistry.getIntegration(id);
...
export const checkIntegrationHealth = (id: string) =>
  lenderIntegrationRegistry.checkIntegrationHealth(id);
export const getIntegrationsByCapability = (capability: string) =>
  lenderIntegrationRegistry.getIntegrationsByCapability(capability);
export const getIntegrationStats = () =>
  lenderIntegrationRegistry.getIntegrationStats();
