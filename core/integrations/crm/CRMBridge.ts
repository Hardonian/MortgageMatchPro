...
export class CRMBridge {
  ...
  async exportLead(
    ...
    try {
      ...
      if (lead.leadScore < integration.configuration.leadScoreThreshold) {
        throw new Error(
          `Lead score ${lead.leadScore} below threshold ${integration.configuration.leadScoreThreshold}`
        );
      }
  ...
  async updateIntegration(
    ...
    try {
      ...
      const validatedConfig =
        CRMIntegrationConfigSchema.parse(updatedIntegration);
  ...
  private async exportToZapier(
    integration: CRMIntegrationConfig,
    lead: CRMLead
  ): Promise<any> {
    ...
    const response = await axios.post(
      integration.configuration.webhookUrl,
      payload,
      {
        timeout: 30000,
        headers: {
  ...
  private async exportToSalesforce(
    integration: CRMIntegrationConfig,
    lead: CRMLead
  ): Promise<any> {
    const client = this.clients.get(integration.id);
  ...
  private async exportToPipedrive(
    integration: CRMIntegrationConfig,
    lead: CRMLead
  ): Promise<any> {
    const client = this.clients.get(integration.id);
  ...
  private async exportToCustom(
    ...
    const payload = this.formatLeadForCustom(
      lead,
      integration.configuration.fieldMapping
