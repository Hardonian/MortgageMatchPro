...
export class AnalyticsBillingService {
  ...
  async getRevenueMetrics(
    ...
    try {
      ...
      // Calculate metrics
      const totalRevenue =
        revenueData?.reduce((sum, r) => sum + r.amount_usd, 0) || 0;
      ...
      const revenueGrowthRate = await this.calculateRevenueGrowth(
        startDate,
        endDate
      );

      // Group by source
      const revenueBySource = this.groupBy(
        revenueData || [],
        "revenue_type",
        "amount_usd"
      );
      const revenueByTier = this.groupBy(
        subscriptionData || [],
        "tier",
        "amount"
      );
      const revenueByRegion = this.groupBy(
        revenueData || [],
        "region",
        "amount_usd"
      );
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get revenue metrics failed",
      };
    }
  }
  ...
  async getUsageMetrics(
    ...
    try {
      ...
      const peakConcurrentUsers = await this.calculatePeakConcurrentUsers(
        startDate,
        endDate
      );
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get usage metrics failed",
      };
    }
  }
  ...
  async getPartnerMetrics(
    ...
    try {
      ...
      const partnerRevenue =
        revenueData?.reduce((sum, r) => sum + r.amount_usd, 0) || 0;
      const averageRevenuePerPartner =
        activePartners > 0 ? partnerRevenue / activePartners : 0;

      // Get top performing partners
      const partnerRevenueMap = new Map<
        string,
        { revenue: number; calls: number }
      >();
      revenueData?.forEach((r) => {
        if (r.partner_id) {
          const existing = partnerRevenueMap.get(r.partner_id) || {
            revenue: 0,
            calls: 0,
          };
      ...
      const partnerChurnRate = await this.calculatePartnerChurnRate(
        startDate,
        endDate
      );
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get partner metrics failed",
      };
    }
  }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Generate billing statement failed",
      };
    }
  }
  ...
  async getPartnerDashboard(
    ...
    try {
      ...
      usageData?.forEach((usage) => {
        const existing = endpointStats.get(usage.endpoint) || {
          calls: 0,
          revenue: 0,
        };
  ...
  private calculateChurnRate(subscriptions: any[]): number {
    ...
    return totalSubscriptions > 0
      ? canceledSubscriptions / totalSubscriptions
      : 0;
  }
  ...
  private calculateAverageResponseTime(usage: any[]): number {
    const totalTime = usage.reduce(
      (sum, u) => sum + (u.response_time_ms || 0),
      0
    );
  ...
  private async calculatePartnerChurnRate(
    startDate: string,
    endDate: string
  ): Promise<number> {
