  ...
  async getUsageStats(
    ...
    try {
      ...
      const averageResponseTime =
        usage.reduce((sum, u) => sum + u.response_time_ms, 0) / totalCalls;
      const successfulCalls = usage.filter(
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get usage stats failed",
      };
    }
  }
  ...
  async getUserApiKeys(userId: string): Promise<{
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Get user API keys failed",
      };
    }
  }
  ...
  async createSubscription(
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Create subscription failed",
      };
    }
  }
}
