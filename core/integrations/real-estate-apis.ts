...
export class RealEstateAPIManager {
  ...
  async searchPropertiesCanada(
    ...
    try {
      ...
      const response = await axios.get(
        "https://api.repliers.io/v1/properties",
        {
          headers: {
      ...
      const properties = response.data.properties.map(
        this.transformRepliersProperty
  ...
  async searchPropertiesUSA(
    ...
    try {
      ...
      const response = await axios.get(
        "https://api.zillow.com/v1/properties/search",
        {
          headers: {
      ...
      const properties = response.data.results.map(
        this.transformZillowProperty
  ...
  async getAffordabilitySuggestions(
    ...
    try {
      ...
      return properties.sort((a, b) => a.price - b.price).slice(0, 10);
    } catch (error) {
  ...
  async getPropertyDetails(
    ...
    try {
      ...
      if (country === "CA") {
        const response = await axios.get(
          `https://api.repliers.io/v1/properties/${mlsId}`,
          {
            headers: {
              Authorization: `Bearer ${this.repliersAPIKey}`,
            },
        ...
        const response = await axios.get(
          `https://api.zillow.com/v1/properties/${mlsId}`,
          {
            headers: {
              "X-RapidAPI-Key": this.zillowAPIKey,
              "X-RapidAPI-Host": "zillow-com1.p.rapidapi.com",
  ...
  private async cacheProperty(property: MLSProperty): Promise<void> {
    try {
      await this.supabaseAdmin.from("mls_property_cache").upsert({
        mls_id: property.mlsId,
        property_data: property,
        location_data: {
          city: property.city,
