...
export const ISO20022PaymentSchema = z.object({
  ...
  paymentInformation: z.object({
    ...
    creditTransferTransactionInformation: z.array(
      z.object({
        paymentId: z.object({
...
export class GlobalPaymentRails {
  ...
  constructor() {
    ...
    this.baseURL =
      process.env.PAYMENT_RAILS_BASE_URL || "https://api.paymentrails.com/v1";
  }
  ...
  async executeSwiftPayment(iso20022Message: ISO20022Payment): Promise<{
    ...
    try {
      const swiftMessage: SwiftMXMessage = {
        messageType: "MT103",
        senderBIC:
          iso20022Message.groupHeader.initiatorParty.identification.bic ||
          "MMPTCAT1",
        receiverBIC:
          iso20022Message.paymentInformation.creditorAgent
        ...
        currency:
          iso20022Message.paymentInformation
            .creditTransferTransactionInformation[0].amount.instructedAmount
            .currency,
        amount:
          iso20022Message.paymentInformation
            .creditTransferTransactionInformation[0].amount.instructedAmount
        ...
        remittanceInfo:
          iso20022Message.paymentInformation
            .creditTransferTransactionInformation[0].remittanceInformation
            .unstructured,
        charges: "OUR",
        regulatoryReporting: [
          {
  ...
  async executeStablecoinTransfer(transfer: {
    ...
    try {
      ...
      const response = await axios.post(
        `${this.baseURL}/stablecoin/transfer`,
        {
  ...
  async executeMultiCurrencySettlement(
    settlements: Array<{
      fromCurrency: string;
      toCurrency: string;
      amount: number;
    ...
    for (const settlement of settlements) {
      ...
      const settlementId = `SETTLE${Date.now()}${Math.random()
        .toString(36)
        .substr(2, 6)}`;
      ...
      try {
        ...
        results.totalExecutionTime = Math.max(
