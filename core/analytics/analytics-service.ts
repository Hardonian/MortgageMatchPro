...
export class AnalyticsService {
  ...
  private initializeEventHandlers(): void {
    ...
    this.eventBus.subscribe(
      AI_EVENT_TYPES.SCORING_REQUESTED,
      this.handleAIScoringRequested.bind(this)
    );
    this.eventBus.subscribe(
      AI_EVENT_TYPES.SCORING_COMPLETED,
      this.handleAIScoringCompleted.bind(this)
    );
    this.eventBus.subscribe(
      AI_EVENT_TYPES.SCORING_FAILED,
      this.handleAIScoringFailed.bind(this)
    );
    ...
    this.eventBus.subscribe(
      BILLING_EVENT_TYPES.PAYMENT_SUCCEEDED,
      this.handlePaymentSucceeded.bind(this)
    );
    this.eventBus.subscribe(
      BILLING_EVENT_TYPES.USAGE_RECORDED,
      this.handleUsageRecorded.bind(this)
    );

    // Track user activity events
    this.eventBus.subscribe(
      "user.session.started",
      this.handleUserSessionStarted.bind(this)
    );
    this.eventBus.subscribe(
      "user.session.ended",
      this.handleUserSessionEnded.bind(this)
    );
    this.eventBus.subscribe(
      "user.page.viewed",
      this.handleUserPageViewed.bind(this)
    );
  }
  ...
  async getRevenueMetrics(tenantId?: string): Promise<RevenueMetrics> {
    const tenants = tenantId
      ? [this.tenantMetrics.get(tenantId)].filter(Boolean)
      : Array.from(this.tenantMetrics.values());

    const totalRevenue = tenants.reduce(
      (sum, tenant) => sum + tenant.totalRevenue,
      0
    );
    const monthlyRecurringRevenue = tenants.reduce(
      (sum, tenant) => sum + tenant.monthlyRecurringRevenue,
      0
    );
    const totalUsers = tenants.reduce(
      (sum, tenant) => sum + tenant.totalUsers,
      0
    );
    ...
    return {
      ...
      churnRate:
        tenants.reduce((sum, tenant) => sum + tenant.churnRate, 0) /
          tenants.length || 0,
  ...
  async getUsageMetrics(tenantId?: string): Promise<UsageMetrics> {
    ...
    const totalAIRequests = tenants.reduce(
      (sum, tenant) => sum + tenant.totalAIRequests,
      0
    );
    const totalUsers = tenants.reduce(
      (sum, tenant) => sum + tenant.totalUsers,
  ...
  async generateReport(
    ...
    switch (type) {
      ...
      case "custom":
        start =
          period?.start || new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        end = period?.end || now;
        break;
    }
  ...
  async getReports(limit = 10): Promise<AnalyticsReport[]> {
    return this.reports
  ...
  async cleanupOldReports(olderThanDays = 90): Promise<void> {
    const cutoff = new Date(Date.now() - olderThanDays * 24 * 60 * 60 * 1000);
