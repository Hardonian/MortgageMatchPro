...
export class AuthService {
  ...
  static async refreshSession(): Promise<{
    ...
    try {
      ...
      return {
        session: null,
        error: { message: "Session refresh failed" } as AuthError,
      };
    }
  }

  // User entitlements management
  static async grantUserEntitlements(
    userId: string,
    tier: "free" | "premium" | "broker"
  ): Promise<void> {
    try {
  ...
  private static getEntitlementsForTier(tier: "free" | "premium" | "broker") {
    const baseEntitlements = [
      {
        feature: "unlimited_calculations",
        type: "subscription",
        expiresAt: null,
        limit: null,
      },
