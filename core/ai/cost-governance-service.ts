...
export class AICostGovernanceService {
   ...
   */
  static async getQuotaPrediction(
    organizationId: string
    ...
    try {
      ...
      const remainingRequests = Math.max(
        0,
        (config.dailyBudget - (usage.aiCost || 0)) / 0.01
      ); // Rough estimate
      const remainingTokens = Math.max(
        0,
        remainingCost / Math.min(...Object.values(config.costPerToken))
      );
  ...
  private static async updateDailyUsageStats(
    ...
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        throw new Error(
          `Failed to fetch existing stats: ${fetchError.message}`
        );
      }
      ...
      if (existingStats) {
        ...
        const updatedStats = {
          ...
          modelBreakdown: {
            ...
            [model]: {
              tokens:
                (existingStats.modelBreakdown[model]?.tokens || 0) + tokensUsed,
              cost: (existingStats.modelBreakdown[model]?.cost || 0) + cost,
              requests:
                (existingStats.modelBreakdown[model]?.requests || 0) + 1,
            },
          },
          updatedAt: new Date().toISOString(),
        }
        ...
        if (updateError) {
          throw new Error(
            `Failed to update AI usage stats: ${updateError.message}`
          );
        }
      } else {
        // Create new stats
        ...
        if (insertError) {
          throw new Error(
            `Failed to create AI usage stats: ${insertError.message}`
          );
        }
      }
    } catch (error) {
  ...
  private static validateCostConfig(config: Partial<AICostConfig>): void {
    ...
    if (
      config.maxTokensPerRequest !== undefined &&
      config.maxTokensPerRequest < 1
    ) {
      throw new Error("Max tokens per request must be at least 1");
    }

    if (
      config.modelAllowlist !== undefined &&
    ...
    if (
      ...
      !config.modelAllowlist?.includes(config.fallbackModel)
    ) {
  ...
  private static getRecommendedAction(
    ...
    dailyBudget: number
  ): "continue" | "throttle" | "block" | "upgrade" {
