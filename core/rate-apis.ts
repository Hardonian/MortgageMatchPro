  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    if (this.state === "OPEN") {
      if (Date.now() - this.lastFailureTime > this.resetTimeout) {
        ...
        throw new Error(
          "Rate API circuit breaker is OPEN - service temporarily unavailable"
        );
      }
    }
...
export class RateService {
  ...
  constructor() {
    ...
    this.freddieMacAPI = new FreddieMacAPI(
      process.env.FREDDIE_MAC_API_KEY || ""
    );
  }
  ...
  async healthCheck(): Promise<{ ratehub: boolean; freddiemac: boolean }> {
    ...
    return {
      ...
      freddiemac:
        freddiemacHealth.status === "fulfilled" && freddiemacHealth.value,
    };
  }
