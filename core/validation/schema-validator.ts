...
export const BusinessRules = {
   ...
   */
  debtToIncomeRatio: (maxRatio = 0.43): BusinessRule => ({
      ...
      if (dti > maxRatio) {
        return {
          ...
          errors: [
            {
              field: "debtToIncomeRatio",
              message: `Debt-to-income ratio ${(dti * 100).toFixed(
                1
              )}% exceeds maximum ${maxRatio * 100}%`,
              code: "DTI_TOO_HIGH",
              value: dti,
            },
          ],
   ...
   */
  loanToValueRatio: (maxRatio = 0.95): BusinessRule => ({
      ...
      if (ltv > maxRatio) {
        return {
          ...
          errors: [
            {
              field: "loanToValueRatio",
              message: `Loan-to-value ratio ${(ltv * 100).toFixed(
                1
              )}% exceeds maximum ${maxRatio * 100}%`,
              code: "LTV_TOO_HIGH",
              value: ltv,
   ...
   */
  minimumDownPayment: (minPercentage = 0.05): BusinessRule => ({
    field: "minimumDownPayment",
    name: "Minimum Down Payment",
    description: `Down payment must be at least ${
      minPercentage * 100
    }% of property value`,
    severity: "error",
    validate: (data: any) => {
      ...
      if (downPaymentPercentage < minPercentage) {
        return {
          ...
          errors: [
            {
              field: "minimumDownPayment",
              message: `Down payment ${(downPaymentPercentage * 100).toFixed(
                1
              )}% is below minimum ${minPercentage * 100}%`,
