...
export class PublicApiService {
  ...
  static async processRequest(
    ...
    try {
      // Validate API key
      const apiKeyInfo = await this.validateApiKey(
        request.apiKey,
  ...
  private static async routeRequest(
    ...
    try {
      switch (endpoint) {
        case "/v1/scenarios":
          return await this.handleScenariosRequest(
            method,
            body,
            request.organizationId

        case "/v1/matches":
          return await this.handleMatchesRequest(
            method,
            body,
            request.organizationId

        case "/v1/reports":
          return await this.handleReportsRequest(
            method,
            body,
            request.organizationId

        case "/v1/rates":
          return await this.handleRatesRequest(
            method,
            body,
            request.organizationId
  ...
  private static shouldSendWebhook(endpoint: string): boolean {
    const webhookEndpoints = ["/v1/scenarios", "/v1/matches", "/v1/reports"];
