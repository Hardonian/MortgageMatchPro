...
export class SREMetricsCollector {
   ...
   */
  async recordMetric(
    metric: Omit<SREMetric, "id" | "timestamp">
  ): Promise<void> {
   ...
   */
  private async checkErrorBudget(
    service: string,
    errorRate: number
  ): Promise<void> {
    try {
  ...
  async updateErrorBudget(
    ...
    try {
      await supabaseAdmin.from("error_budgets").upsert({
        service,
  ...
  async updateSLI(sli: Omit<SLI, "last_updated">): Promise<void> {
    try {
      await supabaseAdmin.from("slis").upsert({
        ...sli,
        last_updated: new Date().toISOString(),
      });
    } catch (error) {
  ...
  async updateSLO(slo: Omit<SLO, "last_updated">): Promise<void> {
    try {
      await supabaseAdmin.from("slos").upsert({
        ...slo,
        last_updated: new Date().toISOString(),
      });
    } catch (error) {
  ...
  private async flushMetrics(): Promise<void> {
    ...
    try {
      await supabaseAdmin.from("sre_metrics").insert(
  ...
  async getMetricsSummary(
    ...
    try {
      ...
      const availabilityMetrics = metrics.filter(
        (m) => m.name === "availability"
      );
      summary.availability =
        availabilityMetrics.length > 0
          ? (availabilityMetrics.reduce((sum, m) => sum + m.value, 0) /
      ...
      summary.mttr =
        mttrMetrics.length > 0
          ? mttrMetrics.reduce((sum, m) => sum + m.value, 0) /
            mttrMetrics.length
          : 0;
...
export const recordAvailability =
  sreMetrics.recordAvailability.bind(sreMetrics);
