...
export class OperationalReadinessManager {
  ...
  async calculateReadinessMetrics(): Promise<OperationalReadinessMetrics> {
    try {
      ...
      const errorBudgetRemaining = Math.max(
        0,
        this.errorBudgetTarget - summary.errorRate
      );
      ...
      const slaCompliance = {
        ...
        regional_availability: Object.values(regionalAvailability).every(
          (av) => av >= 99.0
        ),
      };
      ...
      captureException(error as Error, {
        context: "calculate_readiness_metrics",
      });
      throw error;
    }
  }
  ...
  private async calculateCostPerTransaction(): Promise<number> {
    try {
      ...
      captureException(error as Error, {
        context: "calculate_cost_per_transaction",
      });
      return 0;
    }
  }
   ...
   */
  private async storeReadinessMetrics(
    metrics: OperationalReadinessMetrics
  ): Promise<void> {
    try {
      await supabaseAdmin.from("operational_readiness_metrics").insert({
        ...metrics,
        last_updated: metrics.last_updated.toISOString(),
      });
    } catch (error) {
      captureException(error as Error, {
        context: "store_readiness_metrics",
        metrics,
      });
    }
  }
  ...
  async getDisasterRecoveryPlans(): Promise<DisasterRecoveryPlan[]> {
    try {
      ...
      captureException(error as Error, {
        context: "get_disaster_recovery_plans",
      });
      return [];
    }
  }
   ...
   */
  async upsertDisasterRecoveryPlan(
    ...
    try {
      ...
      captureException(error as Error, {
        context: "upsert_disaster_recovery_plan",
        plan,
      });
      throw error;
    }
  ...
  async getBusinessContinuityModels(): Promise<BusinessContinuityModel[]> {
    try {
      ...
      captureException(error as Error, {
        context: "get_business_continuity_models",
      });
      return [];
    }
  }
   ...
   */
  async upsertBusinessContinuityModel(
    model: Omit<BusinessContinuityModel, "id">
  ): Promise<string> {
    try {
      const { data, error } = await supabaseAdmin
        .from("business_continuity_models")
      ...
      captureException(error as Error, {
        context: "upsert_business_continuity_model",
        model,
      });
      throw error;
    }
   ...
   */
  async upsertVendorContract(
    contract: Omit<VendorContract, "id">
  ): Promise<string> {
    try {
      const { data, error } = await supabaseAdmin
        .from("vendor_contracts")
      ...
      captureException(error as Error, {
        context: "upsert_vendor_contract",
        contract,
      });
      throw error;
    }
  ...
  async checkExpiringContracts(
    ...
    try {
      ...
      return contracts.filter(
        (contract) =>
          contract.renewal_date <= cutoffDate && contract.status === "active"
      );
    } catch (error) {
      captureException(error as Error, {
        context: "check_expiring_contracts",
        daysAhead,
      });
      return [];
    }
    ...
    try {
      ...
      if (metrics.uptime_percentage < this.uptimeTarget) {
        recommendations.push(
          `Uptime is ${metrics.uptime_percentage.toFixed(
            2
          )}%, below target of ${this.uptimeTarget}%`
        );
      }

      if (metrics.mttr_minutes > this.mttrTarget) {
        recommendations.push(
          `MTTR is ${metrics.mttr_minutes.toFixed(
            2
          )} minutes, above target of ${this.mttrTarget} minutes`
        );
      }

      if (metrics.error_budget_remaining < 0.5) {
        recommendations.push(
          `Error budget remaining is ${metrics.error_budget_remaining.toFixed(
            2
          )}%, consider reducing error rate`
        );
      }

      if (expiringContracts.length > 0) {
        recommendations.push(
          `${expiringContracts.length} vendor contracts expiring within 90 days`
        );
      }

      const drPlansNeedingTest = drPlans.filter(
        (plan) => new Date(plan.next_test) <= new Date()
      );
      if (drPlansNeedingTest.length > 0) {
        recommendations.push(
          `${drPlansNeedingTest.length} disaster recovery plans need testing`
        );
      }
      ...
      captureException(error as Error, {
        context: "generate_readiness_report",
      });
      throw error;
    }
  }
  ...
  async initializeDefaultData(): Promise<void> {
    try {
      ...
      await captureMessage(
        "Initialized default operational readiness data",
        "info"
      );
    } catch (error) {
  ...
  startMonitoring(): void {
    ...
    setInterval(async () => {
      try {
        ...
        captureException(error as Error, {
          context: "operational_readiness_monitoring",
        });
      }
    }, 60 * 60 * 1000); // 1 hour
    ...
    setInterval(async () => {
      try {
        ...
        captureException(error as Error, {
          context: "contract_expiry_monitoring",
        });
      }
...
export const calculateReadinessMetrics =
  operationalReadiness.calculateReadinessMetrics.bind(operationalReadiness);
export const generateReadinessReport =
  operationalReadiness.generateReadinessReport.bind(operationalReadiness);
export const getDisasterRecoveryPlans =
  operationalReadiness.getDisasterRecoveryPlans.bind(operationalReadiness);
export const getBusinessContinuityModels =
  operationalReadiness.getBusinessContinuityModels.bind(operationalReadiness);
export const getVendorContracts =
  operationalReadiness.getVendorContracts.bind(operationalReadiness);
