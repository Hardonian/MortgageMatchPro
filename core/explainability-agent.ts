...
export class ExplainabilityAgent {
  ...
  async explainScenario(
    ...
    return this.parseScenarioExplanation(
  ...
  async explainComparison(
    ...
    const prompt = this.buildComparisonExplanationPrompt(
      comparison,
      userContext
    ...
    return this.parseComparisonExplanation(
      response.choices[0].message.content || ""
    );
  }
  ...
  async explainWhatIfAnalysis(
    ...
    const prompt = this.buildWhatIfExplanationPrompt(
      baseScenario,
      modifiedScenario,
      changes
    ...
    return this.parseWhatIfExplanation(
      response.choices[0].message.content || ""
    );
  }
  ...
  async explainCompliance(
    ...
    return this.parseComplianceExplanation(
      response.choices[0].message.content || ""
    );
  }
...
${changes
  .map(
    (c) =>
      `- ${
        c.parameter
      }: $${c.oldValue.toLocaleString()} → $${c.newValue.toLocaleString()}`
  )
  .join("\n")}
...
- Monthly Payment Change: $${(
      modifiedScenario.monthlyPayment - baseScenario.monthlyPayment
    ).toLocaleString()}
- Total Interest Change: $${(
      modifiedScenario.totalInterest - baseScenario.totalInterest
    ).toLocaleString()}
- Qualification Impact: ${
      modifiedScenario.qualificationResult !== baseScenario.qualificationResult
        ? "Changed"
        : "Unchanged"
    }
...
`;
  ...
  private parseScenarioExplanation(content: string) {
    ...
    return {
      summary:
        lines.find(
          (line) => line.includes("Summary") || line.includes("summary")
        ) || "No summary provided",
      keyInsights: lines
      ...
      pros: lines
        .filter(
          (line) => line.toLowerCase().includes("pro") || line.includes("✓")
        )
        .map((line) => line.replace(/^[•\-]\s*/, "")),
      cons: lines
        .filter(
          (line) => line.toLowerCase().includes("con") || line.includes("✗")
        )
        .map((line) => line.replace(/^[•\-]\s*/, "")),
      recommendations: lines
        .filter(
          (line) =>
            line.toLowerCase().includes("recommend") || line.includes("suggest")
        )
        .map((line) => line.replace(/^[•\-]\s*/, "")),
      riskAssessment:
        lines.find((line) => line.toLowerCase().includes("risk")) ||
  ...
  private parseComparisonExplanation(content: string) {
    ...
    return {
      executiveSummary:
        lines.find(
          (line) => line.includes("Executive") || line.includes("Summary")
        ) || "No summary provided",
      detailedComparison:
        lines.find(
          (line) => line.includes("Detailed") || line.includes("Comparison")
        ) || "No detailed comparison provided",
      bestOptionExplanation:
        lines.find(
          (line) => line.includes("Best") || line.includes("Recommended")
        ) || "No best option explanation provided",
      worstOptionExplanation:
        lines.find(
          (line) => line.includes("Worst") || line.includes("Avoid")
        ) || "No worst option explanation provided",
      savingsBreakdown:
        lines.find(
          (line) => line.includes("Savings") || line.includes("Cost")
        ) || "No savings breakdown provided",
      riskAnalysis:
        lines.find(
          (line) => line.includes("Risk") || line.includes("Risk Analysis")
        ) || "No risk analysis provided",
      personalizedRecommendation:
        lines.find(
          (line) => line.includes("Recommendation") || line.includes("Advice")
        ) || "No recommendation provided",
      nextSteps: lines
        .filter((line) => line.includes("Step") || line.includes("Next"))
        .map((line) => line.replace(/^[•\-]\s*/, "")),
    };
  }
  ...
  private parseWhatIfExplanation(content: string) {
    ...
    return {
      changeSummary:
        lines.find(
          (line) => line.includes("Change") || line.includes("Summary")
        ) || "No change summary provided",
      impactAnalysis:
        lines.find(
          (line) => line.includes("Impact") || line.includes("Analysis")
        ) || "No impact analysis provided",
      sensitivityInsights:
        lines.find(
          (line) => line.includes("Sensitivity") || line.includes("Insight")
        ) || "No sensitivity insights provided",
      recommendations: lines
        .filter(
  ...
  private parseComplianceExplanation(content: string) {
    ...
    return {
      complianceStatus:
        lines.find(
          (line) => line.includes("Status") || line.includes("Compliant")
        ) || "No compliance status provided",
      regulatoryExplanation:
        lines.find(
          (line) => line.includes("Regulatory") || line.includes("Requirement")
        ) || "No regulatory explanation provided",
      warnings: lines
        .filter((line) => line.includes("Warning") || line.includes("Issue"))
        .map((line) => line.replace(/^[•\-]\s*/, "")),
      recommendations: lines
        .filter(
          (line) => line.includes("Recommend") || line.includes("Improve")
        )
        .map((line) => line.replace(/^[•\-]\s*/, "")),
    };
  }
