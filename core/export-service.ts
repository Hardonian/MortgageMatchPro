...
export class ExportService {
    ...
    try {
      ...
      const reportData = {
        ...
        scenarios: comparison.scenarios.map((scenario, index) => ({
          ...
          Qualification: scenario.qualificationResult
            ? "Approved"
            : "Not Approved",
          "Risk Factors": scenario.riskFactors.length,
        })),
        amortization: options.includeAmortization
          ? this.generateAmortizationData(comparison)
          : null,
        aiInsights: options.includeAIInsights ? comparison.aiInsights : null,
      };
  ...
  async generateShareLink(
    ...
    try {
      ...
      const { error } = await supabaseAdmin.from("shared_scenarios").insert({
  ...
  async predictFuturePayments(
    ...
    try {
      ...
      const predictions = this.calculateRatePredictions(
        scenario,
        historicalTrends
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to predict future payments",
      };
    }
  }
  ...
  private generateAmortizationData(comparison: ScenarioComparison) {
    const maxMonths = Math.max(
      ...comparison.scenarios.map((s) => s.amortizationSchedule.length)
    );
    const data = [];
    ...
    for (let month = 0; month < maxMonths; month += 12) {
      ...
      comparison.scenarios.forEach((scenario, index) => {
        ...
        if (scheduleEntry) {
          ...
          yearData[`Scenario ${index + 1} Interest`] =
            scheduleEntry.cumulativeInterest;
          yearData[`Scenario ${index + 1} Principal`] =
            scheduleEntry.cumulativePrincipal;
        }
      });
  ...
  private calculateRatePredictions(scenario: any, historicalTrends: any[]) {
    ...
    for (let year = 1; year <= 5; year++) {
      ...
      const avgChange =
        recentTrend.reduce((sum, point, index) => {
          if (index === 0) return 0;
  ...
  private async saveReportMetadata(
    ...
    const { error } = await supabaseAdmin.from("exported_reports").insert({
