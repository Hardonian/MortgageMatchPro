...
export class RegionalRateFeeds {
  ...
  private cache: Map<string, { data: RegionalRate[]; timestamp: number }> =
    new Map();
      ...
      for (const [feedId, feed] of this.feeds) {
        if (feed.countries.includes(country)) {
          try {
            ...
            console.warn(
              `Error fetching rates from feed ${feedId}:`,
              error.message
            );
          }
        }
  ...
  calculateRateDelta(
    ...
    if (delta <= -0.25) {
      ...
      recommendation =
        "This is an excellent rate, significantly below market average";
    } else if (delta <= 0) {
      interpretation = "good";
      ...
      interpretation = "poor";
      recommendation =
        "This rate is above market average, consider shopping around";
    }
  ...
  async getBenchmarkRates(country: "CA" | "US" | "UK"): Promise<{
    ...
    try {
      ...
      const prime =
        rates.find((r) => r.rateType === "prime")?.benchmarkRate || 0;
      const treasury =
        rates.find((r) => r.rateType === "treasury")?.benchmarkRate || 0;
      const centralBank =
        rates.find((r) => r.rateType === "central_bank")?.benchmarkRate || 0;
      const market =
        rates.find((r) => r.rateType === "market")?.benchmarkRate || 0;
  ...
  private generateMockTrends(
    days: number
  ): Array<{ date: string; rate: number; change: number }> {
    const trends = [];
    const baseRate = 5.5;
  ...
  private calculateTrendSummary(
    trends: Array<{ rate: number; change: number }>
  ): {
    trend: "up" | "down" | "stable";
    averageChange: number;
    volatility: number;
    ...
    return [
      {
        ...
        country: "CA",
        currency: "CAD",
        benchmarkRate: 7.2, // Current Bank of Canada rate
...
export const calculateRateDelta = (
  lenderRate: number,
  benchmarkRate: number,
  rateType?: "prime" | "treasury" | "central_bank" | "market"
