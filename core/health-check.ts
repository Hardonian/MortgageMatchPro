...
class HealthCheckSystem {
  ...
  recordRequest(success: boolean): void {
    ...
    this.errorBudget.errorRate =
      this.errorBudget.failedRequests / this.errorBudget.totalRequests;
    this.errorBudget.remaining =
      this.errorBudget.budget - this.errorBudget.errorRate;
...
export const getHealthCheck = (): HealthCheckSystem => {
  if (!healthCheckInstance) {
    throw new Error(
      "Health check not initialized. Call initHealthCheck() first."
    );
  }
  return healthCheckInstance;
};
