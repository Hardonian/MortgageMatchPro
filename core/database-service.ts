...
export class DatabaseService {
  ...
  static async getMortgageCalculations(
    ...
    try {
      ...
      if (data) {
        await redis.setEx(
          cacheKey,
          CACHE_TTL.MORTGAGE_CALCULATIONS,
          JSON.stringify(data)
        );
      }
  ...
  static async saveRateCheck(
    rateCheck: Insert<"rate_checks">
  ): Promise<string | null> {
    try {
      const { data, error } = await supabase
        .from("rate_checks")
  ...
  static async batchInsertMortgageCalculations(
