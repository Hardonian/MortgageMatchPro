...
export class RealtimeService {
    ...
    const channel = supabaseRealtime
      ...
      .on(
        ...
        (payload) => {
          ...
          analytics.trackRealtimeEvent("mortgage_calculation_created", {
            userId,
          });
        }
      )
    ...
    const channel = supabaseRealtime
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
          ...
          callbacks.onError?.(
            new Error("Failed to subscribe to broker updates")
          );
        }
      });
    ...
    const channel = supabaseRealtime
      ...
      .on(
        ...
        (payload) => {
          ...
          analytics.trackRealtimeEvent("user_registered", {
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
          ...
          callbacks.onError?.(
            new Error("Failed to subscribe to system updates")
          );
        }
      });
  ...
  async sendUserNotification(
    ...
    try {
      ...
      analytics.trackRealtimeEvent("notification_sent", {
        userId,
  ...
  async sendBrokerNotification(notification: {
    ...
    try {
      ...
      analytics.trackRealtimeEvent("broker_notification_sent", {
        type: notification.type,
      });
      return true;
    } catch (error) {
  ...
  getConnectionStatus(): "CONNECTED" | "DISCONNECTED" | "CONNECTING" {
    return supabaseRealtime.getChannels().length > 0
      ? "CONNECTED"
      : "DISCONNECTED";
  }
...
export function useRealtime() {
  return {
    ...
    unsubscribeFromUserUpdates:
      realtime.unsubscribeFromUserUpdates.bind(realtime),
    unsubscribeFromBrokerUpdates:
      realtime.unsubscribeFromBrokerUpdates.bind(realtime),
    unsubscribeFromSystemUpdates:
      realtime.unsubscribeFromSystemUpdates.bind(realtime),
    unsubscribeFromRateUpdates:
      realtime.unsubscribeFromRateUpdates.bind(realtime),
