...
class MonetizationSystem {
        ...
        features: [
          {
            ...
            description: "Affordability and rate calculations",
            enabled: true,
          },
          {
            id: "basic_scenarios",
            name: "3 Scenarios",
            description: "Save up to 3 mortgage scenarios",
            enabled: true,
            limit: 3,
          },
          {
        ...
        features: [
          {
            ...
            description: "All calculator features plus refinancing",
            enabled: true,
          },
          {
            id: "unlimited_scenarios",
            name: "Unlimited Scenarios",
            description: "Save unlimited mortgage scenarios",
            enabled: true,
            unlimited: true,
          },
          {
            ...
            description: "Upload and analyze mortgage documents",
            enabled: true,
          },
          {
            id: "priority_support",
  ...
  getUserTier(userId: string): UserTier | undefined {
    const subscription = this.getUserSubscription(userId);
    return subscription
      ? this.userTiers.get(subscription.tierId)
      : this.userTiers.get("free");
  }
  ...
  // Usage tracking
  trackUsage(userId: string, featureId: string, amount = 1): boolean {
  ...
  private async sendReferralEmail(
    email: string,
    referralId: string
  ): Promise<void> {
  ...
  private async applyReferralReward(referral: ReferralProgram): Promise<void> {
    ...
    console.log(
      `Applying ${referral.rewardAmount} ${referral.rewardType} to user ${referral.referrerId}`
    );
  }
...
export const getMonetization = (): MonetizationSystem => {
  if (!monetizationInstance) {
    throw new Error(
      "Monetization not initialized. Call initMonetization() first."
    );
  }
  return monetizationInstance;
};
