...
} from "./experimentation/ab-testing";
...
class PostLaunchSystem {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      "Initializing post-launch growth, intelligence, and monetization systems..."
    );
    ...
    try {
      ...
      initPerformanceMonitor();
      getPerformanceMonitor().setBudget(
        "daily",
        this.config.performance.dailyBudget
      );
      getPerformanceMonitor().setBudget(
        "monthly",
        this.config.performance.monthlyBudget
      );
  ...
  async trackAIQuery(
    ...
    getABTesting().trackExperimentResult(userId, "prompt-style-experiment", {
      success: success ? 1 : 0,
      responseTime,
...
export const initPostLaunchSystem = (
...
export const getPostLaunchSystem = (): PostLaunchSystem => {
  if (!postLaunchInstance) {
    throw new Error(
      "Post-launch system not initialized. Call initPostLaunchSystem() first."
    );
  }
  return postLaunchInstance;
};
...
export const trackAIQuery = async (
  userId: string,
  queryType: string,
  success: boolean,
  responseTime: number,
  ...
  return getPostLaunchSystem().trackAIQuery(
    userId,
    queryType,
    success,
    responseTime,
...
export const collectFeedback = async (
  ...
  return getPostLaunchSystem().collectFeedback(
    userId,
    type,
    sentiment,
    rating,
