  ...
  async cleanupExpiredData(): Promise<{
    ...
    try {
      ...
      const leadCutoff = new Date(
        now.getTime() - this.RETENTION_POLICIES.leadData * 24 * 60 * 60 * 1000
      );
      ...
      const auditCutoff = new Date(
        now.getTime() - this.RETENTION_POLICIES.auditLogs * 24 * 60 * 60 * 1000
      );
      ...
      const consentCutoff = new Date(
        now.getTime() -
          this.RETENTION_POLICIES.personalData * 24 * 60 * 60 * 1000
      );
  ...
  async generateComplianceReport(): Promise<{
    ...
    try {
      ...
      const activeConsents =
        consents?.filter((c) => c.granted && !c.withdrawal_timestamp).length ||
        0;
      const withdrawnConsents =
        consents?.filter((c) => c.withdrawal_timestamp).length || 0;

      const leadsWithinRetention =
        leads?.filter((lead) => new Date(lead.created_at) >= leadCutoff)
          .length || 0;
      const leadsExpired = totalLeads - leadsWithinRetention;

      const auditLogsWithinRetention =
        auditLogs?.filter((log) => new Date(log.timestamp) >= auditCutoff)
          .length || 0;
      const auditLogsExpired =
        (auditLogs?.length || 0) - auditLogsWithinRetention;

      const anonymizationRequests =
        auditLogs?.filter((log) => log.event === "data_anonymized").length || 0;
      const deletionRequests =
        auditLogs?.filter((log) => log.event === "data_deleted").length || 0;
