...
export class ApiService {
  ...
  static async getMortgageCalculations(
    ...
    try {
      ...
      analytics.trackApiCall("get_mortgage_calculations", {
        userId,
  ...
  static async saveMortgageCalculation(
    calculation: Insert<"mortgage_calculations">
  ): Promise<{
    success: boolean;
    data?: { id: string };
    error?: string;
    ...
    try {
      ...
      analytics.trackApiCall("save_mortgage_calculation", {
  ...
  static async getRateChecks(
    ...
    try {
      ...
      analytics.trackApiCall("get_rate_checks", {
        userId,
        count: rateChecks.length,
  ...
  static async saveLead(lead: Insert<"leads">): Promise<{
    ...
    try {
      ...
      await realtime.sendUserNotification(lead.user_id, {
        ...
        title: "Lead Created",
        message:
          "Your lead has been created and will be matched with qualified brokers",
        data: { leadId },
      });
  ...
  static async searchUsers(
    ...
    try {
      ...
      analytics.trackApiCall("search_users", {
        query,
        count: data?.length || 0,
  ...
  static async searchBrokers(
    ...
    try {
      ...
      analytics.trackApiCall("search_brokers", {
        query,
        count: data?.length || 0,
  ...
  // Batch operations
  static async batchSaveMortgageCalculations(
    calculations: Insert<"mortgage_calculations">[]
  ): Promise<{
    success: boolean;
    data?: { saved: number };
    error?: string;
    ...
    try {
      ...
      if (!success) {
        return {
          success: false,
          error: "Failed to save mortgage calculations",
        };
      }

      analytics.trackApiCall("batch_save_mortgage_calculations", {
        count: calculations.length,
  ...
  // Utility functions
  static async checkUserEntitlement(
    userId: string,
    feature: string
  ): Promise<boolean> {
