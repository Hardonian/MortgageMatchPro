...
export const EXPERIMENT_VARIANTS = {
  ...
  ONBOARDING_FLOW: {
    ...
    variant_a: {
      steps: ["welcome", "demo", "goals", "budget"],
      order: "demo_first",
    },
    variant_b: {
      steps: ["welcome", "budget", "goals", "demo"],
      order: "budget_first",
    },
...
export class ExperimentFramework {
  ...
  constructor() {
    this.growthbook = new GrowthBook({
      apiHost:
        process.env.NEXT_PUBLIC_GROWTHBOOK_HOST || "https://cdn.growthbook.io",
  ...
  async trackExperimentEvent(experiment: any, result: any): Promise<void> {
    try {
      ...
      await supabaseAdmin.from("experiment_results").insert({
  ...
  async trackConversion(
    ...
    try {
      ...
      await supabaseAdmin.from("experiment_conversions").insert({
  ...
  async getExperimentResults(experimentId: string): Promise<{
    ...
    try {
      ...
      const variants = this.calculateVariantStats(
        results,
        conversions,
        experiment.variants
      );
  ...
  async updateUserSegment(
    ...
    try {
      ...
      // Store in database
      await supabaseAdmin.from("user_segments").upsert(userSegment);
  ...
  private getVariantConfig(experimentKey: string, variant: string): any {
    const variants =
      EXPERIMENT_VARIANTS[experimentKey as keyof typeof EXPERIMENT_VARIANTS];
    return (
      variants?.[variant as keyof typeof variants] || variants?.control || {}
    );
  }

  private async updateGrowthBookExperiment(
      ...
      return {
        ...
        conversionRate:
          variantResults.length > 0
            ? variantConversions.length / variantResults.length
            : 0,
  ...
  private generateRecommendations(summary: any, significance: any): string[] {
    ...
    if (significance.isSignificant) {
      ...
      recommendations.push(
        "Experiment needs more data for statistical significance"
      );
    }
