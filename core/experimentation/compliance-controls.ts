...
export class ComplianceControlSystem {
  ...
  async checkExperimentCompliance(
    ...
    try {
      ...
      for (const rule of this.complianceRules.values()) {
        ...
        const violation = await this.checkRuleCompliance(
          rule,
          experimentId,
          userId,
          experimentConfig
  ...
  async tagFinancialExplanation(
    ...
    try {
      ...
      await supabaseAdmin.from("tagged_explanations").insert({
  ...
  private async checkRuleCompliance(
    ...
    await supabaseAdmin.from("compliance_events").insert(event);
  ...
  private generateComplianceSummary(
    ...
    return {
      ...
      resolvedViolations,
      resolutionRate:
        totalViolations > 0 ? resolvedViolations / totalViolations : 1,
      totalAuditEvents,
  ...
  private generateComplianceRecommendations(
    ...
    if (frequentViolations.length > 0) {
      recommendations.push(
        `Review and update rules for: ${frequentViolations.join(", ")}`
      );
    }
    ...
    const defaultRules: ComplianceRule[] = [
      {
        ...
        description:
          "Require disclosure when AI affects financial recommendations",
        type: "financial_disclosure",
        conditions: [
          {
            field: "affectsFinancialRecommendations",
            operator: "equals",
            value: true,
          },
        ],
        actions: [
          {
            ...
            config: {
              disclosureType: FINANCIAL_DISCLOSURES.AI_PERSONALIZATION,
            },
          },
        ],
        severity: "high",
        ...
        actions: [
          {
            type: "require_disclosure",
            config: {
              disclosureType: FINANCIAL_DISCLOSURES.EXPERIMENTAL_FEATURES,
            },
          },
        ],
        severity: "medium",
