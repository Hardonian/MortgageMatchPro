...
class ABTestingFramework {
  ...
  getExperimentSummary(experimentId: string): ExperimentSummary | null {
    ...
    const results = this.experimentResults.filter(
    ...
    const statisticalSignificance =
      this.calculateStatisticalSignificance(variantResults);
    const winner = this.determineWinner(variantResults);
    const confidence =
      statisticalSignificance > 0.95 ? 0.95 : statisticalSignificance;
  ...
  private calculateStatisticalSignificance(variantResults: any[]): number {
    ...
    if (controlUsers < 30 || treatmentUsers < 30) {
      return 0;
    }

    // Simplified z-test
    const pooledRate =
      (controlRate * controlUsers + treatmentRate * treatmentUsers) /
      (controlUsers + treatmentUsers);
    const se = Math.sqrt(
      pooledRate * (1 - pooledRate) * (1 / controlUsers + 1 / treatmentUsers)
    );
    const z = Math.abs(treatmentRate - controlRate) / se;

    // Convert z-score to p-value (simplified)
    return z > 1.96 ? 0.95 : z > 1.645 ? 0.9 : 0.8;
  }
...
export const assignUserToVariant = (
  userId: string,
  experimentId: string
): string | null => {
