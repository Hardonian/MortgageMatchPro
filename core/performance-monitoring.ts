...
class PerformanceMonitor {
  ...
  private dailyBudget = 50; // $50/day default
  private monthlyBudget = 1000; // $1000/month default
  ...
  getCacheSummary(): {
    ...
    const averageResponseTime =
      this.cacheMetrics.reduce((sum, m) => sum + m.responseTime, 0) /
      totalRequests;
  ...
  private checkBudgetAlerts(userId: string, additionalCost: number): void {
    ...
    const monthlyPercentage =
      (monthlyUsage.monthlyCost / this.monthlyBudget) * 100;
  ...
  private async sendBudgetAlert(alert: BudgetAlert): Promise<void> {
    ...
    console.log(
      `Budget Alert: ${alert.type} usage at ${alert.percentage.toFixed(
        1
      )}% ($${alert.currentUsage.toFixed(2)} / $${alert.threshold})`
    );
  }
  ...
  optimizeCacheStrategy(): {
    ...
    if (cacheSummary.averageResponseTime > 100) {
      recommendations.push(
        "Cache response times are high, consider optimizing cache implementation"
      );
    }
    ...
    if (performanceSummary.averageResponseTime > 2000) {
      recommendations.push(
        "API response times are high, consider adding more caching layers"
      );
...
export const getPerformanceMonitor = (): PerformanceMonitor => {
  if (!performanceMonitorInstance) {
    throw new Error(
      "Performance monitor not initialized. Call initPerformanceMonitor() first."
    );
  }
  return performanceMonitorInstance;
};
