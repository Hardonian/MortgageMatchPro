...
export class CustomerHealthScoring {
  ...
  async calculateHealthScore(customerId: string): Promise<CustomerHealthScore> {
    try {
      ...
      captureException(error as Error, {
        context: "calculate_health_score",
        customerId,
      });
      throw error;
    }
  ...
  private async gatherCustomerMetrics(
    ...
    try {
      const [usageData, featureData, supportData, paymentData, engagementData] =
        await Promise.all([
      ...
      captureException(error as Error, {
        context: "gather_customer_metrics",
        customerId,
      });
      return {};
    }
   ...
   */
  private async getFeatureAdoptionMetrics(
    customerId: string
  ): Promise<Record<string, any>> {
    try {
      const { data, error } = await supabaseAdmin
      ...
      const adoptionRate =
        totalFeatures > 0 ? (activeFeatures / totalFeatures) * 100 : 0;
      ...
      captureException(error as Error, {
        context: "get_feature_adoption_metrics",
        customerId,
      });
      return {
        total_features: 0,
  ...
  private async getSupportMetrics(
    ...
    try {
      ...
      const avgSatisfaction =
        tickets.length > 0
          ? tickets.reduce(
              (sum, ticket) => sum + (ticket.satisfaction_score || 0),
              0
            ) / tickets.length
          : 0;
      const avgResolutionTime =
        tickets.length > 0
          ? tickets.reduce(
              (sum, ticket) => sum + (ticket.resolution_time || 0),
              0
            ) / tickets.length
          : 0;
      ...
      return {
        ...
        satisfaction_trend: this.calculateTrend(
          tickets.map((t) => t.satisfaction_score || 0)
        ),
      };
    } catch (error) {
      captureException(error as Error, {
        context: "get_support_metrics",
        customerId,
      });
      return {
        avg_satisfaction: 0,
  ...
  private async getPaymentMetrics(
    ...
    try {
      ...
      const paymentHistory = subscriptions.filter(
        (sub) => sub.status === "active" || sub.status === "cancelled"

      const paymentConsistency =
        this.calculatePaymentConsistency(paymentHistory);
      const daysSinceLastPayment = activeSubscription
        ? Math.floor(
            (Date.now() - new Date(activeSubscription.updated_at).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : 0;
    ...
    try {
      ...
      const avgEngagement =
        engagements.length > 0
          ? engagements.reduce((sum, eng) => sum + eng.score, 0) /
            engagements.length
          : 0;

      const engagementTypes = [
        ...new Set(engagements.map((e) => e.engagement_type)),
      ];
      ...
      captureException(error as Error, {
        context: "get_engagement_metrics",
        customerId,
      });
      return {
        avg_engagement: 0,
  ...
  private normalizeUsageScore(usage: any): number {
    ...
    if (usage.last_activity) {
      const daysSinceActivity = Math.floor(
        (Date.now() - new Date(usage.last_activity).getTime()) /
          (1000 * 60 * 60 * 24)
      );
  ...
  private calculatePaymentConsistency(paymentHistory: any[]): number {
    ...
    const totalDuration = paymentHistory.reduce((sum, sub) => {
      ...
      const end =
        sub.status === "cancelled" ? new Date(sub.updated_at) : new Date();
  ...
  ): Promise<CustomerHealthScore["trends"]> {
    try {
      ...
      const usageTrend = this.calculateTrend(
        data.map((d) => d.usage_frequency)
      );
      const engagementTrend = this.calculateTrend(
        data.map((d) => d.engagement_level)
      );
  ...
  private async generateRecommendations(
    ...
    if (healthScore.usage_frequency < 30) {
      recommendations.push(
        "Increase usage frequency through targeted onboarding and feature discovery"
      );
    }

    if (healthScore.feature_adoption < 40) {
      recommendations.push(
        "Improve feature adoption with guided tours and personalized recommendations"
      );
    }

    if (healthScore.support_satisfaction < 60) {
      recommendations.push(
        "Address support satisfaction issues and improve response times"
      );
    }

    if (healthScore.engagement_level < 50) {
      recommendations.push(
        "Boost engagement through gamification and personalized content"
      );
    }

    if (metrics.payments && !metrics.payments.has_active_subscription) {
      recommendations.push(
        "Re-engage customer with subscription offers and value demonstration"
      );
    }

    if (metrics.usage && metrics.usage.unique_days < 5) {
      recommendations.push(
        "Increase session consistency with email campaigns and push notifications"
      );
    }
  ...
  async predictChurnRisk(
    ...
    try {
      ...
      const daysToChurn =
        churnProbability > 0.7 ? 7 : churnProbability > 0.4 ? 30 : 90;
      const predictedChurnDate = new Date(
        Date.now() + daysToChurn * 24 * 60 * 60 * 1000
      );
  ...
  private async storeHealthScore(score: CustomerHealthScore): Promise<void> {
    try {
      await supabaseAdmin.from("customer_health_scores").upsert({
        ...score,
...
export const calculateHealthScore =
  customerHealthScoring.calculateHealthScore.bind(customerHealthScoring);
export const getHealthScore = customerHealthScoring.getHealthScore.bind(
  customerHealthScoring
);
export const getAllHealthScores = customerHealthScoring.getAllHealthScores.bind(
  customerHealthScoring
);
export const getAtRiskCustomers = customerHealthScoring.getAtRiskCustomers.bind(
  customerHealthScoring
);
