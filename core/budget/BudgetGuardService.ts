...
export class BudgetGuardService {
  ...
  async checkBudget(
    ...
    if (estimatedCost > this.config.perRequestLimit) {
      return {
        allowed: false,
        reason: `Request cost ($${estimatedCost.toFixed(
          4
        )}) exceeds per-request limit ($${this.config.perRequestLimit})`,
        status,
      };
    }
  ...
  async getBudgetStatus(): Promise<BudgetStatus> {
    ...
    const monthlyRemaining = Math.max(
      0,
      this.config.monthlyLimit - monthlySpent
    ...
    const isBlocked =
      dailyPercentage >= this.config.blockThreshold * 100 ||
      monthlyPercentage >= this.config.blockThreshold * 100;

    const isAlerted =
      dailyPercentage >= this.config.alertThreshold * 100 ||
   ...
   */
  calculateStripeCost(operation: string, amount: number): number {
    const operationCosts = this.serviceCosts.stripe;
    const percentage =
      operationCosts[operation as keyof typeof operationCosts] || 0.029;

    // Stripe charges percentage + fixed fee
    return amount * percentage + 0.3;
  }

  /**
   * Calculate cost for Supabase operation
   */
  calculateSupabaseCost(operation: string, dataSize: number = 1): number {
    const operationCosts = this.serviceCosts.supabase;
    const costPerUnit =
      operationCosts[operation as keyof typeof operationCosts] || 0.0001;
  ...
  getCostSummaryByService(
    ...
    for (const service in summary) {
      summary[service].averageCost =
        summary[service].totalCost / summary[service].count;
    }
  ...
  private async sendAlert(
    ...
    console.warn(
      `Daily: $${status.dailySpent.toFixed(2)} / $${
        this.config.dailyLimit
      } (${status.dailyPercentage.toFixed(1)}%)`
    );
    console.warn(
      `Monthly: $${status.monthlySpent.toFixed(2)} / $${
        this.config.monthlyLimit
      } (${status.monthlyPercentage.toFixed(1)}%)`
    );
  }
