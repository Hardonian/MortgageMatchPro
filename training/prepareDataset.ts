...
export class DatasetPreparationService {
  ...
  private minExamplesPerCategory = 10;

  constructor(outputDir = './training/datasets') {
  ...
  async validateDataset(dataset: Dataset): Promise<{
    ...
    if (dataset.qualityScore < this.minQualityScore) {
      issues.push(
        `Quality score ${dataset.qualityScore} is below minimum ${this.minQualityScore}`
      );
    ...
    if (minCategoryCount < this.minExamplesPerCategory) {
      issues.push(
        `Some categories have fewer than ${this.minExamplesPerCategory} examples`
      );
      recommendations.push(
        'Collect more examples for underrepresented categories'
      );
    }

    // Check message structure
    const invalidExamples = dataset.examples.filter(
      (example) => !this.isValidMessageStructure(example.messages)
    );
    if (invalidExamples.length > 0) {
      issues.push(
        `${invalidExamples.length} examples have invalid message structure`
      );
  ...
  private async cleanAndAnonymizeData(
    interactions: any[]
  ...
  private filterByQuality(examples: TrainingExample[]): TrainingExample[] {
    return examples.filter((example) => {
      ...
      const hasValidMessages = example.messages.every(
        (msg) => msg.content && msg.content.length > 10
      );

      // Filter by content diversity
      const uniqueContents = new Set(
        example.messages.map((msg) => msg.content)
      );
  ...
  private determineCategory(example: TrainingExample): string {
    ...
    if (templateId.includes('mortgage_recommendation')) {
      return 'mortgage_recommendation';
    }
    if (templateId.includes('rate_explanation')) {
      return 'rate_explanation';
    }
    if (templateId.includes('scenario_comparison')) {
      return 'scenario_comparison';
    }
    if (templateId.includes('affordability')) {
      return 'affordability';
    }
    if (templateId.includes('refinance')) {
      return 'refinance';
    }
  ...
  private createDataset(examples: TrainingExample[]): Dataset {
    const categories = [
      ...new Set(examples.map((ex) => this.determineCategory(ex))),
    ];
  ...
  private calculateQualityScore(examples: TrainingExample[]): number {
    ...
    for (const example of examples) {
      ...
      const avgContentLength =
        example.messages.reduce((sum, msg) => sum + msg.content.length, 0) /
...
if (require.main === module) {
  ...
  service
    .prepareDataset(startDate, endDate, categories)
    .then((dataset) => {
