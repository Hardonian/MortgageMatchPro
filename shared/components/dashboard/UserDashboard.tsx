...
export default function UserDashboard() {
  ...
  const [affordabilityTrends, setAffordabilityTrends] = useState<
    AffordabilityTrend[]
  >([]);
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      setAffordabilityTrends([
        {
          date: "2024-01-01",
          max_affordable: 500000,
          monthly_payment: 2500,
          gds_ratio: 32,
        },
        {
          date: "2024-01-15",
          max_affordable: 520000,
          monthly_payment: 2600,
          gds_ratio: 33,
        },
        {
          date: "2024-02-01",
          max_affordable: 510000,
          monthly_payment: 2550,
          gds_ratio: 32.5,
        },
        {
          date: "2024-02-15",
          max_affordable: 530000,
          monthly_payment: 2650,
          gds_ratio: 33.5,
        },
        {
          date: "2024-03-01",
          max_affordable: 540000,
  ...
  const handleSaveScenario = async (
    ...
    try {
      const { error } = await supabase.from("user_scenarios").insert({
        user_id: user.id,
  ...
  const filteredScenarios = scenarios.filter((scenario) => {
    const matchesSearch = scenario.scenario_name
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesFilter =
      filterType === "all" || scenario.scenario_type === filterType;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Track your mortgage calculations and insights
          </p>
        </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {engagementMetrics?.total_calculations || 0}
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {engagementMetrics?.total_rate_checks || 0}
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {engagementMetrics?.total_scenarios_saved || 0}
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {Math.round(
                        engagementMetrics?.avg_time_spent_minutes || 0
                      )}
                      m
                    </p>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-primary/10 rounded-lg">
                        {scenario.scenario_type === "affordability" && (
                          <Calculator className="h-4 w-4 text-primary" />
                        )}
                        {scenario.scenario_type === "rate_comparison" && (
                          <TrendingUp className="h-4 w-4 text-green-500" />
                        )}
                        {scenario.scenario_type === "amortization" && (
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleToggleFavorite(scenario.id, scenario.is_favorite)
                      }
                    >
                      <Star
                        className={`h-4 w-4 ${
                    ...
                    {scenario.scenario_data.propertyPrice && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {formatCurrency(scenario.scenario_data.propertyPrice)}
                        </span>
                      </div>
                    )}
        ...
        <TabsContent value="trends" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Track how your affordability has changed over time
              </p>
            </CardHeader>
            <CardContent>
              <div className="h-80">
                        ...
                        data={[
                          {
                            ...
                            value: scenarios.filter(
                              (s) => s.scenario_type === "affordability"
                            ).length,
                          },
                          {
                            name: "Rate Comparison",
                            value: scenarios.filter(
                              (s) => s.scenario_type === "rate_comparison"
                            ).length,
                          },
                          {
                            name: "Amortization",
                            value: scenarios.filter(
                              (s) => s.scenario_type === "amortization"
                            ).length,
                          },
                        ]}
                        cx="50%"
              ...
              <CardContent className="space-y-4">
                  ...
                  <p className="text-blue-700">
                    ...
                    {Math.max(
                      ...[
                        ...
                        (type) =>
                          scenarios.filter((s) => s.scenario_type === type)
                            .length
                      )
                    )}{" "}
                    {scenarios.length > 0
                      ? scenarios
                          .reduce((a, b) =>
                            scenarios.filter(
                              (s) => s.scenario_type === a.scenario_type
                            ).length >
                            scenarios.filter(
                              (s) => s.scenario_type === b.scenario_type
                            ).length
                              ? a
                              : b
                          )
                  ...
                  <p className="text-green-700">
                    {scenarios.length > 0
                      ? formatCurrency(
                          scenarios.reduce(
                            (sum, s) =>
                              sum + (s.scenario_data.propertyPrice || 0),
                            0
                          ) / scenarios.length
                        )
                      : "--"}{" "}
                ...
                <div className="p-4 bg-purple-50 rounded-lg">
                  ...
                  <p className="text-purple-700">
                    ...
                    {Math.round(engagementMetrics?.avg_time_spent_minutes || 0)}{" "}
                    minutes per session
                  </p>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    You haven't created any rate comparison scenarios yet.
                    Compare different mortgage rates to find the best deal.
                  </p>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Save your calculations to track trends and compare different
                    scenarios over time.
                  </p>
                </div>
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Check back regularly to see how market changes affect your
                    affordability.
                  </p>
                </div>
              </CardContent>
