...
class AIAutoScale {
  ...
  async analyzeAndScale(): Promise<void> {
    try {
      ...
      const scalingDecision = await this.makeScalingDecision(
        currentMetrics,
        costPrediction
  ...
  private async collectUsageMetrics(): Promise<UsageMetrics> {
    ...
    const apiRequests =
      this.calculateMetric(healthMetrics, "api_requests", "sum") || 0;
    const databaseQueries =
      this.calculateMetric(healthMetrics, "database_queries", "sum") || 0;
    const memoryUsage =
      this.calculateMetric(healthMetrics, "memory_heap_used", "avg") || 0;
    const cpuPercent =
      this.calculateMetric(healthMetrics, "cpu_usage", "avg") || 0;
    const responseTime =
      this.calculateMetric(healthMetrics, "response_time", "avg") || 0;
  ...
  private async analyzeCostTrajectory(): Promise<CostPrediction> {
    ...
    const currentCost =
      historicalData[historicalData.length - 1]?.metric_value || 0;
  ...
  private async makeScalingDecision(
    ...
    const needsScaleUp =
      ...
      currentMetrics.memory_usage_mb >
        (scalingThresholds.scale_up.memory_percent * 1024) / 100 ||
      currentMetrics.response_time_ms >
        scalingThresholds.scale_up.response_time_ms ||
    ...
    const canScaleDown =
      ...
      currentMetrics.memory_usage_mb <
        (scalingThresholds.scale_down.memory_percent * 1024) / 100 &&
      currentMetrics.response_time_ms <
        scalingThresholds.scale_down.response_time_ms &&
      currentMetrics.error_rate < scalingThresholds.scale_down.error_rate;

    // Cost-based scaling decisions
    const costIncreasePercent =
      ((costPrediction.predicted_cost_7d - currentMetrics.cost_usd) /
        currentMetrics.cost_usd) *
      100;
  ...
  private async checkBudgetAlerts(
    ...
    const costIncreasePercent =
      ((costPrediction.predicted_cost_7d - costPrediction.current_cost) /
        costPrediction.current_cost) *
      100;

    if (costIncreasePercent > budgetThreshold) {
      console.log(
        `🚨 Budget alert: ${costIncreasePercent.toFixed(
          1
        )}% cost increase predicted`
  ...
  private async createScalingIssue(decision: ScalingDecision): Promise<void> {
    try {
      const title = `🤖 AI Auto-Scale: ${decision.action
        .replace("_", " ")
        .toUpperCase()}`;
   ...
   */
  private async createCostAlertDiscussion(
    costPrediction: CostPrediction,
    increasePercent: number
  ): Promise<void> {
    try {
      const title = `💰 Cost Alert: ${increasePercent.toFixed(
        1
      )}% Increase Predicted`;
...
### Target Metrics
${
  Object.keys(decision.target_metrics).length > 0
    ? Object.entries(decision.target_metrics)
...
*This discussion was automatically created by the AI Cost Analysis system.*
  ...
  private async storeAnalysisResults(
    ...
    try {
      ...
      await this.supabase.from("ai_analysis_results").insert(analysisData);
   ...
   */
  private performLinearRegression(data: any[]): {
    slope: number;
    intercept: number;
    rSquared: number;
  } {
  ...
  private generateCostRecommendations(
    ...
    if (trend === "increasing") {
      ...
      recommendations.push(
        "Consider upgrading to more efficient instance types"
      );
    }

    if (predicted30d > current * 2) {
      recommendations.push(
        "High cost growth detected - immediate optimization required"
      );
